...
test('diagnostics - channels - builtin - udp', async (t) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('socket').subscribe((message, name) => {
        if (closed) return
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('bind').subscribe((message, name) => {
        if (closed) return
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('message').subscribe((message, name) => {
        if (closed) return
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('send.start').subscribe((message, name) => {
        if (closed) return
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('send.end').subscribe((message, name) => {
        if (closed) return
  ...
  pending.push(
    new Promise((resolve) => {
      let pending = 2
      group.channel('close').subscribe((message, name) => {
        if (closed) return
...
test('diagnostics - channels - builtin - fs', async (t) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('handle').subscribe((message, name) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('handle.open').subscribe((message, name) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('handle.close').subscribe((message, name) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('handle.read').subscribe((message, name) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('handle.write').subscribe((message, name) => {
...
test('diagnostics - channels - builtin - dns', async (t) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('lookup.start').subscribe((message) => {
  ...
  pending.push(
    new Promise((resolve) => {
      group.channel('lookup.end').subscribe((message) => {
        t.equal(typeof message.hostname, 'string', 'message.host is string')
        t.equal(typeof message.family, 'number', 'message.port is number')
        resolve()
      })
