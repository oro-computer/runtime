...
test('vm.runInContext(source) - script refererences', async (t) => {
  ...
  t.deepEqual(
    await result(1, 2, 3),
    [1, 2, 3],
    'function reference returns value'
  )
  ...
  const Class = await vm.runInContext(
    'class Class { method (...args) { return args } }'
  )
...
test('vm.runInContext(source, context) - context', async (t) => {
  ...
  t.equal(
    'function',
    typeof context.functions.hello,
    'typeof context.function.hello === "function"'
  )
  t.equal(
    'hello world',
    await context.functions.hello(),
    'await context.function.hello() === "hello world"'
  )
})
