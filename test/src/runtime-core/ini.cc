...
namespace oro::Tests {
  void ini (Harness& t) {
    t.test("oro::INI::parse", [] (auto t) {
      auto simple = oro::INI::parse(R"INI(
        key = "value"
      )INI");

      t.equals(simple["key"], "value", "simple[key] == value");

      auto sections = oro::INI::parse(R"INI(
        [section-1]
        key = "value"
      ...
      auto subsections = oro::INI::parse(R"INI(
        [section-1]
        key = "value"
        [.subsection]
        key = "value"
      ...
      auto arrays = oro::INI::parse(R"INI(
        [numbers]
        array[] = 1
        array[] = 2
        array[] = 3
      ...
      auto dotsyntax = oro::INI::parse(R"INI(
        [a.b.c.d.e.f]
        g = "value"
      ...
      auto escapedSection = oro::INI::parse(R"INI(
        [section\]]
        key = value
      )INI");
      ...
      auto singleQuotes = oro::INI::parse(R"INI(
        message = 'It\'s fine'
        path = 'C:\\path\\'
      )INI");
