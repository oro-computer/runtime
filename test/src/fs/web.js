import { compareBuffers } from 'oro:util'
...
test('createFile - File.prototype.arrayBuffer()', async (t) => {
  ...
  t.ok(
    compareBuffers(arrayBuffer, buffer) === 0,
    'arrayBuffer() returns actual contents'
  )
  await fd.close()
})
...
test('createFile - File.prototype.text()', async (t) => {
  ...
  t.ok(
    compareBuffers(encoder.encode(text), buffer) === 0,
    'text() returns actual contents'
  )
  await fd.close()
})
...
test('createFile - File.prototype.stream()', async (t) => {
  ...
  t.ok(
    compareBuffers(Buffer.concat(chunks), buffer) === 0,
    'stream() returns actual contents that are chunked'
  )
  await fd.close()
})
...
test('createFileSystemFileHandle', async (t) => {
  ...
  t.ok(
    ...
    'handle.isSameEntry() on same instance'
  )
...
test('createFileSystemDirectoryHandle', async (t) => {
  ...
  t.ok(
    handle && handle instanceof globalThis.FileSystemDirectoryHandle,
    'FileSystemDirectoryHandle instance created'
  )
  ...
  const expected = ['0.txt', '1.txt', '2.txt', 'a.txt', 'b.txt', 'c.txt']

  for (const key of expected) {
    t.ok(
      items[key] instanceof globalThis.FileSystemHandle,
      `'${key}' is FileSystemHandle`
    )
  }
})
