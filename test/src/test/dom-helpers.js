...
test('oro:test sleep and requestAnimationFrame', async (t) => {
  await t.sleep(100)
  await t.requestAnimationFrame()
  await t.requestAnimationFrame('Raf with a comment')
})

test('oro:test wait for', async (t) => {
...
test('oro:test events and clicking', async (t) => {
  const buttonEl = document.createElement('button')
  buttonEl.innerText = 'Push Me'
  buttonEl.id = 'push-me-button'
...
test('oro:test events and typing focus and blur', async (t) => {
  ...
  t.equal(
    inputEl.value,
    'hello world',
    'Input element value was changed by typing'
  )
...
test('oro:test visibility', async (t) => {
  const wrapper = document.createElement('div')
