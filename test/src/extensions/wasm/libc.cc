...
static void initialize_libc_string_tests () {
  ...
  {
    char buffer[8] = {0};
    int written = sprintf(buffer, "%s", "abc");
    test(written == 3);
    test(strcmp(buffer, "abc") == 0);
    test(buffer[3] == '\\0');
  }

  {
    char buffer[4] = {0};
    int written = snprintf(buffer, sizeof(buffer), "%s", "hello");
    test(written == 5);
    test(strcmp(buffer, "hel") == 0);
    test(buffer[3] == '\\0');
  }

  {
    char buffer[1] = {'X'};
    int written = snprintf(buffer, 0, "%s", "noop");
    test(written == 4);
    test(buffer[0] == 'X');
  }

  {
    ...
    char set[] = "xyzod";
    char* match = strpbrk(text, set);
    test(match == text + 4);
    test(strpbrk(text, "XYZ") == NULL);
  }

  {
    char text[] = "terminator";
    char* last = strrchr(text, '\\0');
    test(last == text + strlen(text));
  }

  {
    const char* sample = "character";
    test(strchr(sample, 'a') == sample + 2);
    test(strchr(sample, '\\0') == sample + strlen(sample));
    test(strchr(sample, 'z') == NULL);
  }

  {
    unsigned char data[] = {0x00, 0xff, 0x7f};
    test(memchr(data, '\\0', sizeof(data)) == data);
    test(memchr(data, '\xff', sizeof(data)) == data + 1);
    test(memchr(data, -1, sizeof(data)) == data + 1);
    test(memchr(data, 'c', 2) == NULL);
  }

  {
    char sample[] = "example";
    char* duplicated = strndup(sample, 3);
    test(duplicated != NULL);
    test(strlen(duplicated) == 3);
    test(strcmp(duplicated, "exa") == 0);
    free(duplicated);
  }

  {
    ...
    test(strcmp(token, "alpha") == 0);
    token = strtok(NULL, ",");
    test(strcmp(token, "beta") == 0);
    token = strtok(NULL, ",");
    test(strcmp(token, "gamma") == 0);
    test(strtok(NULL, ",") == NULL);
  }
}
...
static void initialize_libc_ctype_tests () {
  test(isdigit('0') != 0);
  test(isdigit('A') == 0);
  ...
  test(tolower(EOF) == EOF);
  test(toupper(EOF) == EOF);

  test(toascii(0x80) == 0);
  test(toascii(EOF) == EOF);
}
