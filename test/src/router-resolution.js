...
test('router-resolution', async (t) => {
  ...
  const cases = [
    ...
    },
    {
      ...
      redirect: true,
      redirectBodyTest:
        '<meta http-equiv="refresh" content="0; url=\'/router-resolution/a-conflict-index/\'" />',
    ...
    },
    {
      ...
      redirect: true,
      redirectBodyTest:
        '<meta http-equiv="refresh" content="0; url=\'/router-resolution/an-index-file/\'" />',
    ...
    if (testCase.redirect) {
      if (isWindows) {
        ...
        t.equal(
          ...
          `Redirect response url matches ${testCase.redirectUrl} (windows)`
        )
      } else {
        t.equal(
          responseBody,
          testCase.redirectBodyTest,
          `Redirect response matches ${testCase.redirectBodyTest}`
        )
        const extractedRedirectURL = extractUrl(response, responseBody)
        const redirectResponse = await fetch(extractedRedirectURL)
        const redirectResponseBody = (await redirectResponse.text()).trim()
        t.ok(
          redirectResponseBody.includes(testCase.bodyTest),
          `Redirect response body includes ${testCase.bodyTest}`
        )
      }
    } else {
      t.ok(
        responseBody.includes(testCase.bodyTest),
        `response body includes ${testCase.bodyTest}`
      )
    }
  }
})
