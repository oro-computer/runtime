import util, { hasOwnProperty } from 'oro:util'
...
test('util.hasOwnProperty', (t) => {
  ...
  t.ok(
    ...
    'util.hasOwnProperty returns true for own properties'
  )
  t.ok(
    !hasOwnProperty(obj, 'bar'),
    'util.hasOwnProperty returns false for non-own properties'
  )
})
...
test('util.isTypedArray', (t) => {
  ...
  t.ok(
    util.isTypedArray(arr),
    'util.isTypedArray returns true for typed arrays'
  )
  t.ok(
    !util.isTypedArray([]),
    'util.isTypedArray returns false for non-typed arrays'
  )
})
...
test('util.isArrayBufferView', (t) => {
  const arr = new Uint8Array(8)
  t.ok(
    util.isArrayBufferView(arr),
    'util.isArrayBufferView returns true for typed arrays'
  )
  t.ok(
    !util.isArrayBufferView([]),
    'util.isArrayBufferView returns false for arrays'
  )
  t.ok(
    !util.isArrayBufferView({}),
    'util.isArrayBufferView returns false for objects'
  )
})
...
test('util.isAsyncFunction', (t) => {
  const fn = async () => {}
  t.ok(
    util.isAsyncFunction(fn),
    'util.isAsyncFunction returns true for async functions'
  )
  t.ok(
    !util.isAsyncFunction(() => {}),
    'util.isAsyncFunction returns false for non-async functions'
  )
})
...
test('util.isArgumentsObject', (t) => {
  ...
  t.ok(
    util.isArgumentsObject(args),
    'util.isArgumentsObject returns true for arguments objects'
  )
  t.ok(
    !util.isArgumentsObject({}),
    'util.isArgumentsObject returns false for non-arguments objects'
  )
})
...
test('util.isEmptyObject', (t) => {
  ...
  t.ok(
    ...
    'util.isEmptyObject returns true for empty objects'
  )
  t.ok(
    !util.isEmptyObject({ foo: 'bar' }),
    'util.isEmptyObject returns false for non-empty objects'
  )
})
...
test('util.isPlainObject', (t) => {
  ...
  t.ok(
    ...
    'util.isPlainObject returns true for plain objects'
  )
  t.ok(!util.isPlainObject([]), 'util.isPlainObject returns false for arrays')
  t.ok(
    !util.isPlainObject('foo'),
    'util.isPlainObject returns false for strings'
  )
  t.ok(!util.isPlainObject(1), 'util.isPlainObject returns false for numbers')
  t.ok(
    !util.isPlainObject(true),
    'util.isPlainObject returns false for booleans'
  )
  t.ok(!util.isPlainObject(null), 'util.isPlainObject returns false for null')
  t.ok(
    !util.isPlainObject(undefined),
    'util.isPlainObject returns false for undefined'
  )
  t.ok(
    !util.isPlainObject(new Date()),
    'util.isPlainObject returns false for Date objects'
  )
})
...
test('util.isBufferLike', (t) => {
  ...
  t.ok(
    util.isBufferLike(buf),
    'util.isBufferLike returns true for Buffer objects'
  )
  t.ok(
    util.isBufferLike(new Uint8Array(8)),
    'util.isBufferLike returns true for Uint8Array objects'
  )
  ...
  t.ok(
    !util.isBufferLike(undefined),
    'util.isBufferLike returns false for undefined'
  )
})
...
test('util.isFunction', (t) => {
  ...
  t.ok(
    !util.isFunction(undefined),
    'util.isFunction returns false for undefined'
  )
  ...
  const asyncFn = async () => {}
  t.ok(
    util.isFunction(asyncFn),
    'util.isFunction returns true for async functions'
  )
})
...
test('util.isErrorLike', (t) => {
  ...
  t.ok(
    !util.isErrorLike(undefined),
    'util.isErrorLike returns false for undefined'
  )
  ...
  t.ok(
    util.isErrorLike(errLike),
    'util.isErrorLike returns true for objects with message and name properties'
  )
})
...
test('util.isPromiseLike', (t) => {
  ...
  t.ok(
    util.isPromiseLike(promise),
    'util.isPromiseLike returns true for Promise objects'
  )
  t.ok(
    !util.isPromiseLike('foo'),
    'util.isPromiseLike returns false for strings'
  )
  t.ok(!util.isPromiseLike(1), 'util.isPromiseLike returns false for numbers')
  t.ok(
    !util.isPromiseLike(true),
    'util.isPromiseLike returns false for booleans'
  )
  t.ok(!util.isPromiseLike(null), 'util.isPromiseLike returns false for null')
  t.ok(
    !util.isPromiseLike(undefined),
    'util.isPromiseLike returns false for undefined'
  )
  ...
  t.ok(
    util.isPromiseLike(promiseLike),
    'util.isPromiseLike returns true for objects with a then method'
  )
  // oxlint-disable-next-line unicorn/no-thenable
  const notPromiseLike = { then: 'foo' }
  t.ok(
    !util.isPromiseLike(notPromiseLike),
    'util.isPromiseLike returns false for objects with a then property that is not a function'
  )
})

test('util.toProperCase', (t) => {
  t.equal(
    util.toProperCase('foo'),
    'Foo',
    'util.toProperCase returns a string with the first letter capitalized'
  )
  t.equal(
    util.toProperCase('foo bar'),
    'Foo bar',
    'util.toProperCase returns a string with the first letter capitalized'
  )
  t.equal(
    util.toProperCase('foo bar-baz'),
    'Foo bar-baz',
    'util.toProperCase returns a string with the first letter capitalized'
  )
})
...
test('util.clamp', (t) => {
  t.equal(
    util.clamp(0, 0, 1),
    0,
    'util.clamp returns the lower bound if the value is less than the lower bound'
  )
  t.equal(
    util.clamp(1, 0, 1),
    1,
    'util.clamp returns the upper bound if the value is greater than the upper bound'
  )
  t.equal(
    util.clamp(0.5, 0, 1),
    0.5,
    'util.clamp returns the value if it is between the lower and upper bounds'
  )
})
...
test('util.isValidPercentageValue', (t) => {
  t.equal(
    util.isValidPercentageValue('10.5%'),
    true,
    '10.5% is a valid percentage value'
  )
  t.equal(
    util.isValidPercentageValue('100%'),
    true,
    '100% is a valid percentage value'
  )
  t.equal(
    util.isValidPercentageValue('0%'),
    true,
    '0% is a valid percentage value'
  )
  t.equal(
    util.isValidPercentageValue('0.5%'),
    true,
    '0.5% is a valid percentage value'
  )
  t.equal(
    util.isValidPercentageValue('0.00%'),
    true,
    '0.00% is a valid percentage value'
  )

  t.equal(
    util.isValidPercentageValue('-10.5%'),
    false,
    '-10.5% is not a valid percentage value'
  )
  t.equal(
    util.isValidPercentageValue('100.5%'),
    false,
    '100.5% is not a valid percentage value'
  )
  t.equal(
    util.isValidPercentageValue('0x10%'),
    false,
    '0x10% is not a valid percentage value'
  )
})
