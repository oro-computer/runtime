...
test('dgram exports', (t) => {
  ...
  t.ok(
    dgram.Socket.prototype instanceof EventEmitter,
    'dgram.Socket is an EventEmitter'
  )
  t.ok(dgram.Socket.length === 2, 'dgram.Socket accepts two arguments')
  t.ok(dgram.createSocket, 'dgram.createSocket is available')
  t.ok(
    dgram.createSocket.length === 2,
    'dgram.createSocket accepts two arguments'
  )
})
...
test('Socket creation', (t) => {
  ...
  t.ok(
    ...
    'works for dgram.createSocket with string type udp4'
  )
  t.ok(
    dgram.createSocket('udp6'),
    'works for dgram.createSocket with string type udp6'
  )
  t.ok(
  ...
  t.ok(
    ...
    'works for dgram.createSocket with object entry type udp6'
  )
  t.ok(
    new dgram.Socket('udp4'),
    'works for new dgram.Socket with string type udp4'
  )
  t.ok(
    new dgram.Socket('udp6'),
    'works for new dgram.Socket with string type udp6'
  )
  t.ok(
  ...
  t.ok(
    new dgram.Socket({ type: 'udp6' }),
    'works for new dgram.Socket with object entry type udp6'
  )
})
...
test('dgram createSocket, address, bind, close', async (t) => {
  ...
  t.ok(
    server instanceof dgram.Socket,
    'dgram.createSocket returns a dgram.Socket'
  )
  t.throws(
    () => server.address(),
    // eslint-disable-next-line prefer-regex-literals
...
test('udp connected send without specifying port/address', async (t) => {
  if (process.env.ORO_ANDROID_CI) return
  if (os.platform() === 'win32' && process.env.GITHUB_ACTIONS_CI) return
...
test('udp send callback', async (t) => {
  ...
  const result = await new Promise((resolve) => {
    client.send(message, 41237, address, (err) => {
      client.close()
      if (err) return t.fail(err.message)
      resolve(true)
    })
        ...
        server.once('message', (message) => {
          ...
          try {
            ...
            t.ok(
              ...
              'Bad port for disconnected socket'
            )
          }
