...
$defaultRuntimeHome = "$appDataBase\Programs\oro"

$runtimeHomeSource = $null
if ($env:ORO_HOME -and $env:ORO_HOME.Trim() -ne "") {
  $runtimeHomeSource = $env:ORO_HOME
} else {
  $runtimeHomeSource = $defaultRuntimeHome
}

$env:ORO_HOME = $runtimeHomeSource

$ASSET_PATH = $runtimeHomeSource
$SRC_PATH = "$runtimeHomeSource\src"
$LIB_PATH = "$runtimeHomeSource\lib"
$BIN_PATH = "$runtimeHomeSource\bin"
$INCLUDE_PATH = "$runtimeHomeSource\include"
$WORKING_PATH = $OLD_CWD
$WORKING_BUILD_PATH = "$WORKING_PATH\build"
$LIBUV_BUILD_TYPE = "Release"
$LIBUV_TAG = if ($env:LIBUV_GIT_TAG) { $env:LIBUV_GIT_TAG } else { "v1.52.1" }
# see https://docs.microsoft.com/en-us/microsoft-edge/webview2/concepts/versioning
$WEBVIEW2_VERSION = "$webview"
$ORO_BUILD_OPTIONS = "-O2"
$global:git = "git.exe"
...
$logHome = if ($env:LOCALAPPDATA) { $env:LOCALAPPDATA } else { $env:USERPROFILE }
$logfile="$logHome\oro_install_ps1.log"
...
Function Build-OrocToPathTaskBlock {
  param([string]$binPath)
  $regPath = (Get-ItemProperty -Path 'Registry::HKCU\Environment' -Name Path).Path

  Write-Log "h" "# Checking for oroc.exe in PATH..."
  ...
  $prompt = "Do you want to add oroc.exe to your user PATH? [y/N]"
  $task = [string]{
    Write-Log "h" "# Adding oroc.exe to PATH..."
...
Function Prompt-AddOrocToPath {
  ...
  $prompt, $task = $(Build-OrocToPathTaskBlock $BIN_PATH)
...
Function Build-VCRuntimeInstallBlock() {
  ...
  $prompt = "$installer is required to run oroc.exe, proceed with install from Microsoft? y/[n]?"
  ...
  Write-Output $prompt
  Write-Output $t
  Write-Output "oroc.exe will be unable to run without $installer."
}
...
if ($shbuild) {
  ...
  } elseif (($env:CI -ne "true") -and ($env:GITHUB_ACTIONS -ne "true") -and ($env:ORO_SKIP_PATH_PROMPT -ne "1")) {
    Prompt-AddOrocToPath
  }
