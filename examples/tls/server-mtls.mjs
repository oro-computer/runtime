// Mutual TLS echo server using oro:tls.
// Run with:
...
async function main () {
  ...
  const server = createServer(
    { cert, key, ca, requestCert: true, alpnProtocols: ['http/1.1'] },
    (socket) => {
  ...
  server.on('secureConnectionReady', ({ clientId }) => {
    console.log('mTLS secure for client', clientId)
  })
  server.on('error', (err) => console.error('server error:', err))
  server.on('close', () => console.log('server closed'))

  server.listen(9444, '127.0.0.1', 128, (err) => {
    if (err) console.error('listen error:', err)
    else console.log('mTLS echo server listening on 127.0.0.1:9444')
  })

  const handleShutdown = (signal) => {
    console.log(`${signal} received, shutting down mTLS server...`)
    try {
      server.close(() => {
