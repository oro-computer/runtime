...
namespace oro::runtime::tls {
  namespace {
        ...
        switch (entry.dwAltNameChoice) {
          ...
          case CERT_ALT_NAME_IP_ADDRESS:
            if (entry.IPAddress.cbData == 4 || entry.IPAddress.cbData == 16) {
              ...
              if (InetNtopA(entry.IPAddress.cbData == 4 ? AF_INET : AF_INET6, const_cast<void*>(reinterpret_cast<const void*>(entry.IPAddress.pbData)), buf, static_cast<DWORD>(sizeof(buf)))) {
    ...
    inline Vector<String> decodeSANs(const PCCERT_CONTEXT cert) {
      Vector<String> sans;
      if (!cert) return sans;
    ...
    inline HCERTSTORE createRootStore(const String& pem) {
      ...
      while ((pos = pem.find(header, pos)) != std::string::npos) {
        ...
        trimWhitespace(body);
        DWORD derSize = 0;
        if (!CryptStringToBinaryA(body.c_str(), 0, CRYPT_STRING_BASE64, nullptr, &derSize, nullptr, nullptr) || derSize == 0) {
          pos = end + footer.size();
          continue;
        }
        std::vector<unsigned char> der(derSize);
        if (!CryptStringToBinaryA(body.c_str(), 0, CRYPT_STRING_BASE64, der.data(), &derSize, nullptr, nullptr) || derSize == 0) {
          pos = end + footer.size();
          continue;
        }
        der.resize(derSize);
        ...
        if (ctx) {
          if (CertAddCertificateContextToStore(store, ctx, CERT_STORE_ADD_REPLACE_EXISTING, nullptr)) added = true;
    ...
    inline DWORD protocolMaskServer(const String& minVersion, const String& maxVersion) {
      auto mapVersion = [](const String& v) -> DWORD {
        if (v == "TLSv1" || v == "1.0") return SP_PROT_TLS1_0_SERVER;
        if (v == "TLSv1.1" || v == "1.1") return SP_PROT_TLS1_1_SERVER;
        if (v == "TLSv1.2" || v == "1.2") return SP_PROT_TLS1_2_SERVER;
      #ifdef SP_PROT_TLS1_3_SERVER
        if (v == "TLSv1.3" || v == "1.3") return SP_PROT_TLS1_3_SERVER;
      #endif
        return 0;
      };
      DWORD minMask = mapVersion(minVersion);
      DWORD maxMask = mapVersion(maxVersion);
  ...
  struct Server::Impl {
    ...
    DWORD requestedFlags = ASC_REQ_SEQUENCE_DETECT | ASC_REQ_REPLAY_DETECT | ASC_REQ_CONFIDENTIALITY | ASC_REQ_EXTENDED_ERROR | ASC_REQ_ALLOCATE_MEMORY | ASC_REQ_STREAM;
    DWORD contextFlags = 0;
    HCERTSTORE rootStore = nullptr;
    std::vector<PCCERT_CONTEXT> certChain;
  ...
  void Server::close() {
    ...
    schannel::freeCertificateChain(this->impl->certChain);
  ...
  static HRESULT importServerCertificate(Server::Impl* impl, const String& certPem, const String& keyPem, const String& passphrase) {
    ...
    schannel::freeCertificateChain(impl->certChain);
    if (impl->keyHandle) {
    ...
    impl->certChain = std::move(certs);
    impl->keyHandle = key;
    impl->keyProvider = provider;
    impl->keyImported = true;
    return S_OK;
  }
  ...
  int Server::setup(const Options& opts) {
    ...
    schannel::freeCertificateChain(c->certChain);
    if (c->keyHandle) {
    ...
    if (opts.cert.empty() || opts.key.empty()) {
      ...
      c->lastErrorMessage = "TLS server certificate and key are required";
      return -1;
    }

    HRESULT hrIdentity = importServerCertificate(c, opts.cert, opts.key, opts.keyPassphrase);
    if (FAILED(hrIdentity)) {
      c->lastErrorCode = hrIdentity;
      c->lastErrorMessage = describeIdentityError(hrIdentity, false);
      return -1;
    }
    c->lastErrorCode = S_OK;
    c->lastErrorMessage.clear();
    ...
    cred.cCreds = static_cast<DWORD>(c->certChain.size());
    cred.paCred = c->certChain.empty() ? nullptr : c->certChain.data();
    cred.dwFlags = SCH_USE_STRONG_CRYPTO;
    cred.grbitEnabledProtocols = protocolMaskServer(opts.minVersion, opts.maxVersion);
    ...
    c->requestClientCert = opts.requestClientCert;
    if (c->requestClientCert) {
      c->requestedFlags |= ASC_REQ_MUTUAL_AUTH;
    } else {
      c->requestedFlags &= ~ASC_REQ_MUTUAL_AUTH;
    }

    TimeStamp expiry{};
    SECURITY_STATUS status = AcquireCredentialsHandleW(nullptr, const_cast<wchar_t*>(UNISP_NAME_W), SECPKG_CRED_INBOUND, nullptr, &cred, nullptr, nullptr, &c->cred, &expiry);
    if (status != SEC_E_OK) {
      HRESULT hr = status;
    ...
    c->verifyFlags = 0;
    c->verifyInfo.clear();
    c->verifiedSans.clear();
    c->verifiedSubject.clear();
    c->haveSizes = false;
    c->manualVerify = c->manualVerify || !opts.ca.empty();
    c->verifyDone = false;

    return 0;
  }

  void Server::attachTransport(types::SharedPointer<oro::runtime::tcp::Socket> socket) {
  ...
  static void evaluateClientCert(Server::Impl* impl) {
    ...
    if (QueryContextAttributes(&impl->ctx, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &cert) != SEC_E_OK) {
      if (impl->requestClientCert) {
        impl->verifyFlags = CERT_TRUST_IS_PARTIAL_CHAIN;
        impl->verifyInfo = "Client certificate not provided";
      }
      impl->verifyDone = true;
      return;
    }
    impl->verifiedSubject = nameFromCert(cert);
    impl->verifiedSans = decodeSANs(cert);
    ...
    if (!impl->manualVerify) {
      ...
      if (!CertGetCertificateChain(chainEngine ? chainEngine : HCCE_CURRENT_USER, cert, nullptr, cert->hCertStore, &chainPara, 0, nullptr, &chainContext) || !chainContext) {
        ...
        message = (flags == CERT_TRUST_NO_ERROR) ? std::string() : "Client certificate validation failed";
        CertFreeCertificateChain(chainContext);
      }
      if (chainEngine) CertFreeCertificateChainEngine(chainEngine);
    }
  ...
  int Server::driveHandshake() {
    ...
    if (!c->haveContext) {
      status = AcceptSecurityContext(&c->cred, nullptr, c->encryptedIn.empty() ? nullptr : &inDesc, c->requestedFlags, SECURITY_NATIVE_DREP, &c->ctx, &outDesc, &c->contextFlags, &c->expiry);
      if (status == SEC_E_OK || status == SEC_I_CONTINUE_NEEDED) {
        c->haveContext = true;
      }
    } else {
      inDesc.cBuffers = 2;
      status = AcceptSecurityContext(&c->cred, &c->ctx, c->encryptedIn.empty() ? nullptr : &inDesc, c->requestedFlags, SECURITY_NATIVE_DREP, nullptr, &outDesc, &c->contextFlags, &c->expiry);
    }

    if (outBuffer.pvBuffer && outBuffer.cbBuffer > 0 && this->transport) {
      this->transport->write(reinterpret_cast<const char*>(outBuffer.pvBuffer), outBuffer.cbBuffer, [](int){});
    ...
    if (status == SEC_E_OK || status == SEC_I_CONTEXT_EXPIRED) {
      ...
      if (!c->haveSizes) {
        if (QueryContextAttributes(&c->ctx, SECPKG_ATTR_STREAM_SIZES, &c->sizes) == SEC_E_OK) c->haveSizes = true;
      }
      size_t extra = (inBuffers[1].BufferType == SECBUFFER_EXTRA) ? inBuffers[1].cbBuffer : 0;
      removeConsumed(c->encryptedIn, extra);
      evaluateClientCert(c);
      return 0;
    }

    if (status == SEC_I_CONTINUE_NEEDED || status == SEC_E_INCOMPLETE_MESSAGE || status == SEC_I_RENEGOTIATE) {
    ...
    if (status == SEC_E_INCOMPLETE_CREDENTIALS) {
      return SSL_ERROR_WANT_READ;
    }
  ...
  int Server::writeApp(const unsigned char* data, size_t len) {
    ...
    while (offset < len) {
      ...
      this->transport->write(reinterpret_cast<const char*>(buffer.data()), sendLen, [](int){});
      offset += chunk;
  ...
  int Server::getPeerSubject(types::String& subject) {
    ...
    evaluateClientCert(this->impl);
    subject = this->impl->verifiedSubject;
    return 0;
  }
  ...
  int Server::getSessionInfo(types::String& protocol, types::String& cipher) {
    ...
    if (QueryContextAttributes(&this->impl->ctx, SECPKG_ATTR_CONNECTION_INFO, &info) == SEC_E_OK) {
      switch (info.dwProtocol) {
        case SP_PROT_TLS1_0_SERVER: protocol = "TLSv1.0"; break;
        case SP_PROT_TLS1_1_SERVER: protocol = "TLSv1.1"; break;
        case SP_PROT_TLS1_2_SERVER: protocol = "TLSv1.2"; break;
      #ifdef SP_PROT_TLS1_3_SERVER
        case SP_PROT_TLS1_3_SERVER: protocol = "TLSv1.3"; break;
      #endif
        default: protocol.clear(); break;
      }
    }
  ...
  int Server::shutdown() {
    ...
    outDesc.ulVersion = SECBUFFER_VERSION;
    status = AcceptSecurityContext(&this->impl->cred, &this->impl->ctx, nullptr, this->impl->requestedFlags, SECURITY_NATIVE_DREP, nullptr, &outDesc, &this->impl->contextFlags, &this->impl->expiry);
    if (status == SEC_E_OK || status == SEC_I_CONTEXT_EXPIRED) {
      if (out.pvBuffer && out.cbBuffer > 0) {
        this->transport->write(reinterpret_cast<const char*>(out.pvBuffer), out.cbBuffer, [](int){});
  ...
  const types::String& Server::lastErrorMessage() const {
    static types::String empty;
