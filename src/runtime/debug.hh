#ifndef ORO_RUNTIME_RUNTIME_DEBUG_H
#define ORO_RUNTIME_RUNTIME_DEBUG_H
...
// Apple
#ifndef debug
// define `oro_runtime_os_log_debug` (macos/ios)
#if defined(ORO_CLI)
#  define oro_runtime_os_log_debug(...)
#else
static os_log_t ORO_RUNTIME_OS_LOG_DEBUG = nullptr;
// wrap `os_log*` functions for global debugger
#define oro_runtime_os_log_debug(format, fmt, ...) ({                       \
  if (!ORO_RUNTIME_OS_LOG_DEBUG) {                                          \
    static auto userConfig = oro::runtime::config::getUserConfig();            \
    static auto bundleIdentifier = userConfig["meta_bundle_identifier"];       \
    ORO_RUNTIME_OS_LOG_DEBUG = os_log_create(                               \
...
#define debug(format, ...) ({                                                  \
  ...
  oro_runtime_os_log_debug("%{public}@", format, ##__VA_ARGS__);            \
})
#endif // `debug`
#endif  // `__APPLE__`
...
// Windows
#if ORO_RUNTIME_PLATFORM_WINDOWS && defined(DEBUG)
