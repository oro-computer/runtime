...
@implementation OROApplicationDelegate
 ...
 didFinishLaunchingWithOptions: (NSDictionary*) launchOptions
{
  ...
  for (const auto& arg : split(self.app->runtime.userConfig["application_argv"], ',')) {
    if (arg.find("--test") == 0) {
      isTest = true;
    }
  ...
  auto getConfigValue = [&](const String& key) -> String {
    if (self.app->runtime.userConfig.count(key) > 0) {
      return self.app->runtime.userConfig[key];
    }
    return String("");
  };
  ...
  self.app->runtime.serviceWorkerManager.init("oro://" + self.app->runtime.userConfig["meta_bundle_identifier"], {
  ...
  if (isDebugEnabled() && port > 0 && host.size() > 0) {
    ...
    defaultWindow->navigate(
      "oro://" + self.app->runtime.userConfig["meta_bundle_identifier"] + self.app->runtime.userConfig["webview_root"]
    );
  } else {
    defaultWindow->navigate(
      "oro://" + self.app->runtime.userConfig["meta_bundle_identifier"] + "/index.html"
    );
  }
