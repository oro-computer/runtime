#ifndef ORO_RUNTIME_CORE_H
#define ORO_RUNTIME_CORE_H
        ...
        public:
          ...
          bool remove (const Observer& observer) {
            ...
            while (it != this->observers.end()) {
              if (it->id == 0 || it->id == observer.id) {
                it = this->observers.erase(it);
                if (!removed && it != this->observers.end() && it->id == observer.id) {
                  continue;
                }
                if (!removed && observer.id != 0) {
                  removed = true;
                }
              } else {
                ++it;
