...
namespace {
  ...
  void setEnvValue (const String& name, const String& value) {
...
namespace oro::runtime::env {
  ...
  void set (const char* name) {
    const auto assignment = String(name);
    const auto delimiter = assignment.find('=');
    if (delimiter == String::npos) {
      return;
    }

    const auto key = assignment.substr(0, delimiter);
    const auto value = assignment.substr(delimiter + 1);
    set(key, value);
  }
}
