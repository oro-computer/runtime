...
using oro::runtime::window::Dialog;
...
- (void) resizeSubviewsWithOldSize: (NSSize) oldSize {
  ...
  const auto w = reinterpret_cast<OROWindow*>(window->window);
...
#if (!ORO_RUNTIME_PLATFORM_IOS_SIMULATOR) || (ORO_RUNTIME_PLATFORM_IOS && __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_15)
-                                      (void) webView: (WKWebView*) webView
