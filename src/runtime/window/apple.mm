...
static dispatch_queue_t queue = dispatch_queue_create(
  "oro.runtime.app.queue",
  qos
);
#endif

@implementation OROWindow
...
@implementation OROWindowDelegate
         ...
         didReceiveScriptMessage: (WKScriptMessage*) scriptMessage
  {
    ...
    if (msg.name == "application.exit" || msg.name == "process.exit") {
      ...
      if (code > 0) {
        oro::cli::notify(SIGTERM);
      } else {
        oro::cli::notify(SIGUSR2);
      }
    }
  #endif
  ...
  - (OROWebView*) webView: (OROWebView*) webview
...
- (BOOL) windowShouldClose: (OROWindow*) _ {
  auto window = (Window*) objc_getAssociatedObject(self, "window");
...
namespace oro::runtime::window {
      ...
      dialog(this)
  {
    ...
    this->windowDelegate = [OROWindowDelegate new];
    ...
    this->webview.customUserAgent = [NSString
      stringWithFormat: @("Mozilla/5.0 (Macintosh; Intel Mac OS X %d_%d_%d) AppleWebKit/605.1.15 (KHTML, like Gecko) OroRuntime/%s"),
        processInfo.operatingSystemVersion.majorVersion,
        processInfo.operatingSystemVersion.minorVersion,
        processInfo.operatingSystemVersion.patchVersion,
        oro::runtime::version::VERSION_STRING.c_str()
    ];
    ...
    this->webview.customUserAgent = [NSString
      stringWithFormat: @("Mozilla/5.0 (iPhone; CPU iPhone OS %d_%d_%d like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.2 Mobile/15E148 Safari/604.1 OroRuntime/%s"),
      processInfo.operatingSystemVersion.majorVersion,
      processInfo.operatingSystemVersion.minorVersion,
      processInfo.operatingSystemVersion.patchVersion,
      oro::runtime::VERSION_STRING.c_str()
    ];
  ...
  #elif ORO_RUNTIME_PLATFORM_IOS
    this->window = [OROWindow.alloc initWithFrame: frame];
    this->viewController = [OROWebViewController new];
...
#if ORO_RUNTIME_PLATFORM_MACOS
    if (this->window) {
      [this->window setLevel: enabled ? NSFloatingWindowLevel : NSNormalWindowLevel];
    }
#elif ORO_RUNTIME_PLATFORM_IOS
    // No-op on iOS
#endif
  ...
  #if ORO_RUNTIME_PLATFORM_MACOS
    if (this->window) {
      return [this->window level] >= NSFloatingWindowLevel;
    }
    return false;
  #elif ORO_RUNTIME_PLATFORM_IOS
    return false;
