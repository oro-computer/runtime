...
namespace oro::runtime::window {
  Manager::Manager (context::RuntimeContext& context)
    : context(context),
      windows(ORO_RUNTIME_MAX_WINDOWS + ORO_RUNTIME_MAX_WINDOWS_RESERVED)
  {}
  ...
  SharedPointer<Manager::ManagedWindow> Manager::getWindowForBridge (
    ...
    SharedPointer<ManagedWindow> result = nullptr;
    {
      Lock lock(this->mutex);
  ...
  SharedPointer<Manager::ManagedWindow> Manager::getOrCreateWindow (int index) {
    ...
    defaults.followSystemTheme = this->options.followSystemTheme;
    defaults.preferDarkTheme = this->options.preferDarkTheme;
    return this->getOrCreateWindow(index, defaults);
  }
  ...
  void Manager::destroyWindow (int index) {
    ...
    const auto bridge = static_cast<runtime::Runtime&>(this->context).bridgeManager.get(index);
    {
      ...
      if (window != nullptr) {
        ...
        // On GTK, a programmatic close (gtk_window_close) may not synchronously
        // destroy the native window. Keep the ManagedWindow alive until the
        // GTK "destroy" signal runs (it schedules a follow-up destroyWindow)
  ...
  SharedPointer<Manager::ManagedWindow> Manager::createWindow (
    ...
    debug(
      "window::Manager::createWindow: index=%d screen=%dx%d "
      "config(defaultWidth=\"%s\", defaultHeight=\"%s\") "
      "options(width=%.1f, height=%.1f) -> size=%.1fx%.1f",
      options.index,
      screen.width,
      screen.height,
      this->options.defaultWidth.c_str(),
      this->options.defaultHeight.c_str(),
      options.width,
      options.height,
      width,
      height
    ...
    const auto& runtime = static_cast<runtime::Runtime&>(this->context);
    const bool cdpDebugEnabled = (
      runtime.userConfig.contains("cdp_remote_debugging_port") ||
      env::has("ORO_REMOTE_DEBUGGING_PORT") ||
      env::has("ORO_RUNTIME_REMOTE_DEBUGGING_PORT")
    );
    windowOptions.debug = isDebugEnabled() || options.debug || cdpDebugEnabled;
    ...
    windowOptions.origin = webview::Origin("oro://" + windowOptions.userConfig["meta_bundle_identifier"] + "/");
    ...
    window->setWeakSelf(window);
    const auto weak = window->getWeakSelf();
    bridge->navigateHandler = [weak] (const auto& url) {
    ...
    bridge->evaluateJavaScriptHandler = [weak] (const auto& source) {
      const auto self = weak.lock();
      if (self == nullptr) {
        return;
      }
    ...
    static_cast<runtime::Runtime&>(this->context).services.cdp.onWindowCreated(options.index);

    #if ORO_RUNTIME_PLATFORM_ANDROID
    if (window->getOptions().headless) {
  ...
  JSON::Object Manager::ManagedWindow::json () const {
    ...
    JSON::Any isDarkModeValue;

  #if ORO_RUNTIME_PLATFORM_LINUX
    const bool effectiveDarkMode = this->options.followSystemTheme
      ? this->isDarkMode
      : this->options.preferDarkTheme;
    isDarkModeValue = JSON::Boolean(effectiveDarkMode);
  #else
    if (this->options.followSystemTheme) {
      isDarkModeValue = JSON::Null();
    } else {
      isDarkModeValue = JSON::Boolean(this->options.preferDarkTheme);
    }
  #endif
    ...
    return JSON::Object::Entries {
      ...
      {"followSystemTheme", this->options.followSystemTheme},
      {"preferDarkTheme", this->options.preferDarkTheme},
  ...
  int Manager::getNextWindowIndex (bool reserved) const {
    if (reserved) {
      for (
        int i = ORO_RUNTIME_MAX_WINDOWS;
        i < ORO_RUNTIME_MAX_WINDOWS + ORO_RUNTIME_MAX_WINDOWS_RESERVED;
        ++i
      ) {
  ...
  int Manager::getRandomWindowIndex (bool reserved) const {
    if (reserved) {
      size_t remaining = ORO_RUNTIME_MAX_WINDOWS_RESERVED;
      while (remaining--) {
        const auto index = crypto::randint(
          ORO_RUNTIME_MAX_WINDOWS,
          ORO_RUNTIME_MAX_WINDOWS + ORO_RUNTIME_MAX_WINDOWS_RESERVED - 1
        );
    ...
    size_t remaining = ORO_RUNTIME_MAX_WINDOWS;
    while (remaining--) {
      const auto index = crypto::randint(
        0,
        ORO_RUNTIME_MAX_WINDOWS - 1
      );
