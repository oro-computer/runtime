...
namespace oro::runtime::window {
          ...
          Microsoft::WRL::Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>([=, this](
            ...
            do {
              ...
              if (settings2 != nullptr) {
                LPWSTR currentUserAgent = nullptr;
                String userAgentString;

                if (SUCCEEDED(settings2->get_UserAgent(&currentUserAgent)) && currentUserAgent != nullptr) {
                  userAgentString = convertWStringToString(WString(currentUserAgent));
                }

                if (currentUserAgent != nullptr) {
                  CoTaskMemFree(currentUserAgent);
                }

                userAgentString = trim(replace(userAgentString, "OroRuntime/", ""));

                if (userAgentString.find("OroRuntime/") == String::npos) {
                  if (!userAgentString.empty()) {
                    userAgentString += " ";
                  }

                  userAgentString += "OroRuntime/" + String(oro::runtime::version::VERSION_STRING);
                }

                if (!userAgentString.empty()) {
                  const auto updatedUserAgent = convertStringToWString(userAgentString);
                  settings2->put_UserAgent(updatedUserAgent.c_str());
                }
              }
            ...
            //
            // This event only fires for server certificate failures, so it can
            // allow pinned certificates when the platform would otherwise
            // reject them, but cannot reject otherwise-valid certificates.
            do {
              ICoreWebView2_14* webview14 = nullptr;
              ...
              if (
                FAILED(this->webview->QueryInterface(IID_PPV_ARGS(&webview14))) ||
                webview14 == nullptr
              ) {
                break;
              }
              ...
              webview14->add_ServerCertificateErrorDetected(
                Microsoft::WRL::Callback<ICoreWebView2ServerCertificateErrorDetectedEventHandler>([=, this](
                  ICoreWebView2* sender,
                  ICoreWebView2ServerCertificateErrorDetectedEventArgs* args
                ) -> HRESULT {
                  if (args == nullptr || this->bridge == nullptr) {
                    return S_OK;
                  }
                  ...
                  String requestUri;
                  {
                    LPWSTR requestUriW = nullptr;
                    if (
                      FAILED(args->get_RequestUri(&requestUriW)) ||
                      requestUriW == nullptr
                    ) {
                      return S_OK;
                    }

                    requestUri = convertWStringToString(WString(requestUriW));
                    CoTaskMemFree(requestUriW);
                  }

                  const auto components = URL::Components::parse(requestUri);
                  ...
                  if (
                    FAILED(args->get_ServerCertificate(&certificate)) ||
                    certificate == nullptr
                  ) {
                  ...
                  String pem;
                  {
                    ...
                    pem = convertWStringToString(WString(pemW));
                    CoTaskMemFree(pemW);
                  }
                  ...
                  const auto stripWhitespace = [](String input) -> String {
                    String output;
                  ...
                  const auto header = String("-----BEGIN CERTIFICATE-----");
                  const auto footer = String("-----END CERTIFICATE-----");

                  String body;
                  const auto start = pem.find(header);
                  if (start != String::npos) {
                    const auto bodyStart = start + header.size();
                    const auto end = pem.find(footer, bodyStart);
                    if (end != String::npos && end > bodyStart) {
                  ...
                  const auto derString = decode(body);
                  Vector<uint8_t> der(derString.begin(), derString.end());

                  if (der.empty() || der.size() > static_cast<size_t>(std::numeric_limits<DWORD>::max())) {
                    args->put_Action(COREWEBVIEW2_SERVER_CERTIFICATE_ERROR_ACTION_CANCEL);
                    return S_OK;
                  }
  ...
  void Window::close (int code) {
    ...
    for (auto managed : app->runtime.windowManager.windows) {
      if (managed == nullptr || managed->index == this->index) {
        continue;
      }
      ...
      managed->eval(getEmitToRenderProcessJavaScript("windowclosed", json.str()));
    }
  }
  ...
  void Window::navigate (const String& url) {
    ...
    app->dispatch([=, this] {
      if (this->webview == nullptr || !this->isReadyForNavigation) {
        return;
      }
  ...
  bool Window::isAlwaysOnTop () {
    if (this->window) {
      const auto exStyle = GetWindowLongPtr(this->window, GWL_EXSTYLE);
  ...
  void Window::setMenu (const String& menuSource, const bool& isTrayMenu) {
    ...
    NOTIFYICONDATAW nid = {};
      ...
      nid.uCallbackMessage = WM_ORO_TRAY;
      nid.hIcon = icon;
    } else {
