...
namespace oro::runtime::window {
  Window::Window (SharedPointer<bridge::Bridge> bridge, const Window::Options& options)
    ...
    this->size.width = options.width > 0 ? static_cast<int>(options.width) : 0;
    this->size.height = options.height > 0 ? static_cast<int>(options.height) : 0;
    this->hasPendingSize = this->size.width > 0 && this->size.height > 0;

    if (
      !options.backgroundColorLight.empty() ||
      !options.backgroundColorDark.empty()
    ) {
      auto useDarkMode = options.preferDarkTheme;

      if (options.followSystemTheme) {
        useDarkMode = CallClassMethodFromAndroidEnvironment(
          attachment.env,
          Boolean,
      ...
      auto backgroundColor = useDarkMode
      ...
      if (!backgroundColor.empty()) {
        ...
        this->hasPendingBackgroundColor = true;
      }
    }
  ...
  void Window::setSize (int width, int height, int _) {
    this->size.width = width;
    this->size.height = height;
    this->hasPendingSize = width > 0 && height > 0;
  ...
  void Window::setAlwaysOnTop (bool) {
...
extern "C" {
  ...
  void ANDROID_EXTERNAL(window, Window, onReady) (
    ...
    app->dispatch([window]() {
      if (window->hasPendingTitle) {
        window->setTitle(window->title);
      }

      if (window->hasPendingSize) {
        window->setSize(window->size.width, window->size.height);
      }

      if (window->hasPendingPosition) {
        window->setPosition(window->position.x, window->position.y);
      }

      if (window->hasPendingBackgroundColor) {
        window->setBackgroundColor(window->backgroundColor);
      }
  ...
  jstring ANDROID_EXTERNAL(window, Window, getBundleIdentifier) (
    ...
    auto userConfig = window->getOptions().userConfig;
