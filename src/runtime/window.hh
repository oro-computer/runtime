#ifndef ORO_RUNTIME_WINDOW_H
#define ORO_RUNTIME_WINDOW_H
...
#ifndef ORO_RUNTIME_MAX_WINDOWS
#define ORO_RUNTIME_MAX_WINDOWS 64
#endif

#ifndef ORO_RUNTIME_MAX_WINDOWS_RESERVED
#define ORO_RUNTIME_MAX_WINDOWS_RESERVED 32
#endif

#if ORO_RUNTIME_PLATFORM_WINDOWS
#define WM_HANDLE_DEEP_LINK WM_APP + 1
#define WM_ORO_TRAY WM_APP + 2
#endif
...
namespace oro::runtime::window {
  ...
  class Window {
    public:
      ...
      void setWeakSelf (const SharedPointer<Window>& self) {
        this->weakSelf = self;
      }

      std::weak_ptr<Window> getWeakSelf () const {
      ...
      /**
       * If `true`, when handling the `oro:` scheme,
       * the bridge will prefer an applicable service worker before requesting
       * resources.
       */
      bool shouldPreferServiceWorker = false;
        ...
        */
        ...
        /**
         * If `true`, the window tracks the host desktop theme and updates
         * its dark/light mode state accordingly.
         */
        bool followSystemTheme = true;

        /**
         * If `true`, the window prefers a dark appearance when
         * `followSystemTheme` is disabled.
         */
    ...
    protected:
      ...
      Atomic<uint64_t> lastDomFocusEvalMs = 0;
      Atomic<uint64_t> lastDomBlurEvalMs = 0;
      static inline uint64_t _now_ms () {
        using namespace std::chrono;
        return duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count();
      }
      inline bool evalDomFocusThrottled () {
        const auto now = _now_ms();
        if (now - this->lastDomFocusEvalMs.load(std::memory_order_relaxed) < 50) {
          return false;
        }
        this->lastDomFocusEvalMs.store(now, std::memory_order_relaxed);
        this->eval("window.focus()");
        return true;
      }
      inline bool evalDomBlurThrottled () {
        const auto now = _now_ms();
        if (now - this->lastDomBlurEvalMs.load(std::memory_order_relaxed) < 50) {
          return false;
        }
        this->lastDomBlurEvalMs.store(now, std::memory_order_relaxed);
    ...
    public:
      ...
      /**
       * The cached title of the window.
       */
      String title;

      /**
       * True when the title should be applied after the native window becomes ready.
       */
      bool hasPendingTitle = false;

      /**
       * True when the size should be applied after the native window becomes ready.
       */
      bool hasPendingSize = false;

      /**
       * True when the position should be applied after the native window becomes ready.
       */
      bool hasPendingPosition = false;

      /**
       * True when the background color should be applied after the native window becomes ready.
       */
      bool hasPendingBackgroundColor = false;
    ...
    #if ORO_RUNTIME_PLATFORM_IOS
      OROWebViewController* viewController = nullptr;
    #endif

    #if ORO_RUNTIME_PLATFORM_APPLE
      OROWindowDelegate* windowDelegate = nullptr;
      OROWindow* window = nullptr;
