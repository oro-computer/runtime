...
@implementation OROUserNotificationCenterDelegate
  ...
  if ([response.actionIdentifier isEqualToString: UNNotificationDefaultActionIdentifier]) {
    ...
    action = "dismiss";
  } else if (response.actionIdentifier != nullptr) {
    action = String(response.actionIdentifier.UTF8String);
  } else {
    action = "default";
  }
...
namespace oro::runtime::core::services {
  ...
  bool Notifications::start () {
    ...
    this->userNotificationCenterDelegate = [OROUserNotificationCenterDelegate new];
  ...
  bool Notifications::show (const ShowOptions& options, const ShowCallback callback) {
    ...
    if (options.badge.size() > 0) {
      userInfo[@"badge"] = @(options.badge.c_str());
    }

    if (options.dir.size() > 0) {
      userInfo[@"dir"] = @(options.dir.c_str());
    }

    if (options.data.size() > 0) {
      userInfo[@"data"] = @(options.data.c_str());
    }

    if (options.actions.size() > 0) {
      userInfo[@"actions"] = @(options.actions.c_str());
    }

    userInfo[@"renotify"] = @(options.renotify);
    userInfo[@"requireInteraction"] = @(options.requireInteraction);
    ...
    if (options.actions.size() > 0) {
      try {
        const auto parsedActions = JSON::parse(options.actions);
        if (parsedActions.type == JSON::Type::Array) {
          auto nativeActions = [NSMutableArray array];

          for (const auto& entry : parsedActions.as<JSON::Array>()) {
            ...
            auto nativeAction = [UNNotificationAction
              actionWithIdentifier: @(identifier.c_str())
              title: @(title.c_str())
          ...
          if (nativeActions.count > 0) {
            NSString* categoryIdentifier = options.category.size() > 0
              ? @(options.category.c_str())
              : [NSString stringWithFormat: @"oro.notification.%s", options.id.c_str()];

            content.categoryIdentifier = categoryIdentifier;

            NSSet<UNNotificationCategory*>* categories = [notificationCenter notificationCategories];
            NSMutableSet<UNNotificationCategory*>* updatedCategories = [categories mutableCopy];
          ...
          #if !__has_feature(objc_arc)
            [updatedCategories release];
          #endif
          } else if (options.category.size() > 0) {
            content.categoryIdentifier = @(options.category.c_str());
          }
        } else if (options.category.size() > 0) {
          content.categoryIdentifier = @(options.category.c_str());
        }
      } catch (...) {
        if (options.category.size() > 0 && content.categoryIdentifier == nil) {
          content.categoryIdentifier = @(options.category.c_str());
        }
      }
    } else if (options.category.size() > 0) {
    ...
    const auto success = CallClassMethodFromAndroidEnvironment(
      ...
      "("
        ...
        "Ljava/lang/String;" // actionsJSON
        "Ljava/lang/String;" // dataJSON
        "Z" // requireInteraction
        "Z" // renotify
      ")Z",
      ...
      attachment.env->NewStringUTF(options.actions.c_str()),
      attachment.env->NewStringUTF(options.data.c_str()),
