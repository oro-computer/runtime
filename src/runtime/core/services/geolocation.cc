...
@implementation OROLocationObserver
- (id) init {
  self = [super init];
  self.delegate = [[OROLocationManagerDelegate alloc] initWithLocationObserver: self];
...
- (int) watchPositionForIdentifier: (NSInteger) identifier
  ...
  OROLocationPositionWatcher* watcher = nullptr;
  BOOL exists = NO;

  for (OROLocationPositionWatcher* existing in self.locationWatchers) {
  ...
  if (!watcher) {
    watcher = [OROLocationPositionWatcher
...
- (BOOL) clearWatch: (NSInteger) identifier {
  for (OROLocationPositionWatcher* watcher in self.locationWatchers) {
...
@implementation OROLocationManagerDelegate
...
- (void) locationManager: (CLLocationManager*) locationManager
  ...
  for (id item in locationRequestCompletions) {
    auto completion = (void (^)(NSError*, CLLocation*)) item;
    completion(nullptr, locations.firstObject);
  ...
  for (OROLocationPositionWatcher* watcher in self.locationObserver.locationWatchers) {
...
- (void) locationManager: (CLLocationManager*) locationManager
  ...
  // Propagate error to pending one-shot location requests
  auto locationRequestCompletions = [NSArray arrayWithArray: self.locationObserver.locationRequestCompletions];
  for (id item in locationRequestCompletions) {
    auto completion = (void (^)(NSError*, CLLocation*)) item;
...
- (void)            locationManager: (CLLocationManager*) locationManager
  ...
  // Propagate deferred error to pending one-shot location requests
  auto locationRequestCompletions = [NSArray arrayWithArray: self.locationObserver.locationRequestCompletions];
  for (id item in locationRequestCompletions) {
    auto completion = (void (^)(NSError*, CLLocation*)) item;
    completion(error, nullptr);
...
namespace oro::runtime::core::services {
  ...
  #if ORO_RUNTIME_PLATFORM_APPLE
    this->locationObserver = [OROLocationObserver new];
    this->locationPositionWatcher = [OROLocationPositionWatcher new];
    this->locationManagerDelegate = [OROLocationManagerDelegate new];
