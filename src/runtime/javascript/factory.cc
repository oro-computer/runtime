...
namespace oro::runtime::javascript {
  String createJavaScript (const String& name, const String& source) {
    static constexpr const char* prefix =
      ";(async () => {                                                       \n"
      "  if (typeof globalThis.__RUNTIME_INIT_NOW__ !== 'number') {          \n"
      ...
      "  let globals = null;                                                 \n"
      "  try {                                                               \n"
      "    globals = await import('oro:internal/globals');                   \n"
      "  } catch (err) {                                                     \n"
      "    try {                                                             \n"
      "      const bundle = globalThis.__args?.config?.meta_bundle_identifier;\n"
      "      if (bundle) {                                                   \n"
      "        globals = await import(`oro://${bundle}/oro/internal/globals.js`);\n"
      "      }                                                               \n"
      "    } catch {}                                                        \n"
      "                                                                      \n"
      "    if (!globals) {                                                   \n"
      "      console.error(                                                  \n"
      "        'Failed to import oro:internal/globals',                       \n"
      "        { href: globalThis.location?.href, origin: globalThis.origin },\n"
      "        err                                                           \n"
      "      );                                                              \n"
      "    }                                                                 \n"
      "  }                                                                   \n"
    ...
    static constexpr const char* suffix =
      ...
      "                                                                      \n"
      "  try {                                                               \n"
      "    if (globals && typeof globals.get === 'function') {               \n"
      "      await globals.get('RuntimeExecution').runInAsyncScope(run);     \n"
      "    } else {                                                          \n"
      "      await run();                                                    \n"
      "    }                                                                 \n"
      "  } catch (err) {                                                     \n"
    ...
    String out;
    const auto src = trim(source);
    out.reserve(strlen(prefix) + src.size() + strlen(suffix) + name.size() + 8);
    out += prefix;
    out += src;
    out += suffix;
    out += name;
