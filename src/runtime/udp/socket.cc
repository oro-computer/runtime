...
namespace oro::runtime::udp {
  ...
  int Socket::bind (const String& address, int port, bool reuseAddr) {
    ...
    if (this->isUDP()) {
      // Populate sockaddr_storage as IPv4 or IPv6 based on address
      if (address.find(':') != String::npos) {
        auto* in6 = reinterpret_cast<struct sockaddr_in6*>(&this->addr);
        if ((err = uv_ip6_addr((char *) address.c_str(), port, in6))) {
          return err;
        }
      } else {
        auto* in4 = reinterpret_cast<struct sockaddr_in*>(&this->addr);
        if ((err = uv_ip4_addr((char *) address.c_str(), port, in4))) {
          return err;
        }
      }
  ...
  int Socket::rebind () {
    ...
    Lock lock(this->mutex);
    memset((void *) &this->addr, 0, sizeof(struct sockaddr_storage));
  ...
  void Socket::send (
    ...
    if (!this->isConnected()) {
      ...
      if (address.find(':') != String::npos) {
        auto* in6 = reinterpret_cast<struct sockaddr_in6*>(&this->addr);
        err = uv_ip6_addr((char *) address.c_str(), port, in6);
      } else {
        auto* in4 = reinterpret_cast<struct sockaddr_in*>(&this->addr);
        err = uv_ip4_addr((char *) address.c_str(), port, in4);
      }
  ...
  int Socket::recvstart (Socket::UDPReceiveCallback receiveCallback) {
    ...
    auto receive = [](
      ...
      if (nread == UV_ENOTCONN) {
        // Free allocated buffer if present before stopping receive
        if (buf && buf->base) {
          delete[] buf->base;
        }
        socket->recvstop();
  ...
  int Socket::recvstop () {
    ...
    if (this->isUDP()) {
      // Stop libuv from delivering further datagrams
      err = uv_udp_recv_stop(reinterpret_cast<uv_udp_t*>(&this->handle));
      if (err < 0) {
        return err;
      }
    }
  ...
  int Socket::resume () {
    ...
    if (this->isBound()) {
      const bool shouldRestartRecv = (this->receiveCallback != nullptr);
      this->removeState(SOCKET_STATE_UDP_RECV_STARTED);
      ...
      // If previously receiving, restart receive on resume
      if (shouldRestartRecv) {
        const int rerr = this->recvstart();
        if (rerr < 0 && rerr != UV_EALREADY && rerr != UV_EBUSY) {
          return rerr;
        }
      }
    } else if (this->isConnected()) {
      const bool shouldRestartRecv = (this->receiveCallback != nullptr);
      const auto remoteAddr = this->remote.address;
      const auto remotePort = this->remote.port;

      if (!remoteAddr.empty() && remotePort > 0) {
        err = this->connect(remoteAddr, remotePort);
        if (err) {
          return err;
        }
      ...
      if (this->isBound()) {
        ...
        if (!uv_is_closing(reinterpret_cast<uv_handle_t*>(&this->handle))) {
          uv_close((uv_handle_t *) &this->handle, [](uv_handle_t* h) {
            auto socket = static_cast<Socket*>(h->data);
            if (socket) {
        ...
        // Close the connected UDP handle to fully pause network IO.
        Lock lock(this->mutex);
        if (!uv_is_closing(reinterpret_cast<uv_handle_t*>(&this->handle))) {
          uv_close(reinterpret_cast<uv_handle_t*>(&this->handle), [](uv_handle_t* h) {
            auto socket = static_cast<Socket*>(h->data);
            if (socket) {
              socket->removeState((socket_state_t) (
                SOCKET_STATE_UDP_BOUND |
