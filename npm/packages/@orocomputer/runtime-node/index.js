...
//
class API {
  static #cliVersionPrefix = '--oroc-version=v'
  static #cliVersionPattern = /v(\d+\.\d+\.\d+)/
  ...
  constructor () {
    ...
    process.on('uncaughtException', (err) => {
      this.#write(
        `ipc://stderr?value=${encodeURIComponent(err.stack || String(err))}`
      )
    })
    ...
    for (const arg of process.argv) {
      if (arg.startsWith(API.#cliVersionPrefix)) {
        const [major, minor, patch] = arg
          .match(API.#cliVersionPattern)?.[1]
          .split('.')
          .map(Number) ?? [0, 0, 0]
  ...
  #checkVersion (major, minor, patch) {
    if (
      major < API.#minimalMajorVersion ||
      minor < API.#minimalMinorVersion ||
      patch < API.#minimalPatchVersion
    ) {
      throw new Error(
        `@orocomputer/runtime-node requires at least version 0.1.0, got ${major}.${minor}.${patch}`
      )
    }
  }
  ...
  #parse (data) {
    ...
    try {
      ...
      console.error(
        `Unable to parse stdin message ${err.code} ${err.message.slice(0, 100)} (${dataStart}...${dataEnd})`
      )
      throw new Error(
        `Unable to parse stdin message ${err.code} ${err.message.slice(0, 20)}`
      )
    }
