...
function initializeXHRIntercept () {
  ...
  const patched = Symbol.for('oro.runtime.ipc.XMLHttpRequest.patched')
  ...
  Object.assign(globalThis.XMLHttpRequest.prototype, {
    ...
    async send (body) {
      ...
      if (
        ...
        if (/put|post|patch/i.test(method) && typeof body !== 'undefined') {
...
function getRequestResponse (request, options) {
  ...
  const desiredResponseType =
    options?.desiredResponseType ?? expectedResponseType
  ...
  if (responseType === 'arraybuffer') {
    ...
    const json = parseJSON(response)
    if (
      (isPlainObject(json?.data) && json?.source) ||
      isPlainObject(json?.err)
    ) {
      response = json
    }
  }
...
export function maybeMakeError (error, caller) {
  const errors = {
    ...
    InternalError,
    DOMException: getErrorClass('DOMException'),
  ...
  if (
    code &&
    ErrnoError.errno?.constants &&
    (-code) in ErrnoError.errno.strings
  ) {
 ...
 * @ignore
 */
export const kDebugEnabled = Symbol.for('oro.runtime.ipc.debug.enabled')
...
if (debug.enabled && globalThis.__args?.env?.ORO_DEBUG_IPC) {
  // eslint-disable-next-line
...
export class Message extends URL {
  ...
  static from (input, params, bytes = null) {
    ...
    if (typeof input === 'string' && params) {
      return new this(
        `${protocol}//${input}?${new URLSearchParams(params)}`,
        bytes
      )
    }
  ...
  static isValidInput (input) {
    ...
    return (
      string.startsWith(`${protocol}//`) && string.length > protocol.length + 2
    )
  }
...
export class Result {
  ...
  static from (
    ...
    const data =
      !err && result?.data !== null && result?.data !== undefined
        ? result.data
        : !err && !id && !result?.source
            ? (result?.err ?? result)
            : null
  ...
  constructor (id, err, data, source, headers) {
    ...
    this.source =
      typeof source === 'string' && source.length > 0 ? source : this.source
...
export class IPCSearchParams extends URLSearchParams {
  ...
  constructor (params, nonce = null) {
    ...
    super({
      ...
      seq: serializedParams?.seq ?? 'R' + nextSeq++
    })

    if (serializedParams && typeof serializedParams === 'object') {
      for (const [key, entryValue] of Object.entries(serializedParams)) {
    ...
    // @ts-ignore
    const runtimeFrameMeta = globalThis.document
      ? globalThis.document.querySelector('meta[name=runtime-frame-source]')
      : null

    const runtimeFrameSource = runtimeFrameMeta?.content ?? ''
...
export function sendSync (command, value = '', options = null, buffer = null) {
  ...
  // Back-compat: many callers pass the request body as the 3rd argument.
  const isBody = (input) =>
    isBufferLike(input) || typeof input === 'string' || Array.isArray(input)

  if (buffer == null && isBody(options)) {
    buffer = options
    options = null
  } else if (isBody(options) && isPlainObject(buffer)) {
    const tmp = buffer
    buffer = options
    options = tmp
  }
  ...
  const pending = new Promise((resolve) => {
    if (options?.timeout) {
      timeout = setTimeout(
        () => {
          resolve(
            Result.from(null, new TimeoutError('ipc.write timed out'), command)
          )
          request.abort()
        },
      ...
      timeout = setTimeout(
        () => {
          resolve(
            Result.from(
              null,
              new TimeoutError('ipc.request timed out'),
              command
            )
          )
          request.abort()
...
export class IPCMessagePort extends MessagePort {
  // `IPCMessagePort` instances "owned" by this environment
  ...
  static create (options = null) {
    ...
    // hidden state
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.id')] = id
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.rx')] = rx
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.tx')] = tx
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.token')] = token
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.closed')] = false
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.started')] = false
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.transferred')] =
      options?.transferred ?? false
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.eventTarget')] = eventTarget
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessage')] = null
    port[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessageerror')] = null
  ...
  get id () {
    /** @type {string|null} */
    return this[Symbol.for('oro.runtime.ipc.IPCMessagePort.id')] ?? null
  }

  get started () {
    /** @type {boolean} */
    return this[Symbol.for('oro.runtime.ipc.IPCMessagePort.started')] ?? false
  }

  get closed () {
    /** @type {boolean} */
    return this[Symbol.for('oro.runtime.ipc.IPCMessagePort.closed')] ?? false
  }

  get onmessage () {
    /** @type {(function(MessageEvent):any)|null} */
    return this[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessage')] ?? null
  }

  set onmessage (onmessage) {
    const current = this[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessage')]
    ...
    if (typeof current === 'function') {
      ...
      this[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessage')] = null
    }

    if (typeof onmessage === 'function') {
      this.addEventListener('message', onmessage)
      this[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessage')] = onmessage
      if (!this.started) {
        this.start()
      }
    }
  ...
  get onmessageerror () {
    return (
      this[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessageerror')] ?? null
    )
  }

  set onmessageerror (onmessageerror) {
    const current =
      this[Symbol.for('oro.runtime.ipc.IPCMessagePort.onmessageerror')]
    const port = IPCMessagePort.ports.get(this.id)
  ...
  start () {
    ...
    this[Symbol.for('oro.runtime.ipc.IPCMessagePort.started')] = true
  }
  ...
  close () {
    ...
    this[Symbol.for('oro.runtime.ipc.IPCMessagePort.closed')] = true
  ...
  postMessage (message, optionsOrTransferList) {
    ...
    const tx = this[Symbol.for('oro.runtime.ipc.IPCMessagePort.tx')]
    const handle =
      this[Symbol.for('oro.runtime.ipc.IPCMessagePort.handlePostMessage')]
    const transfers = new Set(options.transfer)
    const serializedMessage = serialize(
      findIPCMessageTransfers(transfers, message)
    )
  ...
  addEventListener (...args) {
    ...
    const port = IPCMessagePort.ports.get(this.id)
    const eventTarget =
      this[Symbol.for('oro.runtime.ipc.IPCMessagePort.eventTarget')]
  ...
  [Symbol.for('oro.runtime.ipc.IPCMessagePort.handlePostMessage')] (
    _message,
    _options = null
  ) {
    return true
  ...
  [Symbol.for('oro.runtime.serialize')] () {
    if (this[Symbol.for('oro.runtime.ipc.IPCMessagePort.transferred')]) {
      return {
        __type__: 'IPCMessagePort',
        transferred: true,
        rx: this[Symbol.for('oro.runtime.ipc.IPCMessagePort.rx')]?.name ?? null,
        tx: this[Symbol.for('oro.runtime.ipc.IPCMessagePort.tx')]?.name ?? null,
        id: this.id
      }
    }
    return {
  ...
  [Symbol.for('oro.runtime.gc.finalizer')] () {
    return {
      args: [this.id],
      handle (id) {
        if (IPCMessagePort.ports.has(id)) {
...
export class IPCMessageChannel extends MessageChannel {
  ...
  static #connect (port1, port2) {
    ...
    port1[Symbol.for('oro.runtime.ipc.IPCMessagePort.rx')].addEventListener(
      'message',
      (event) => {
        const rx = port2[Symbol.for('oro.runtime.ipc.IPCMessagePort.rx')]
        if (
          rx &&
          port1[Symbol.for('oro.runtime.ipc.IPCMessagePort.transferred')]
        ) {
    ...
    port2[Symbol.for('oro.runtime.ipc.IPCMessagePort.rx')].addEventListener(
      'message',
      (event) => {
        const rx = port1[Symbol.for('oro.runtime.ipc.IPCMessagePort.rx')]
        if (
          rx &&
          port2[Symbol.for('oro.runtime.ipc.IPCMessagePort.transferred')]
        ) {
          rx.dispatchEvent(new MessageEvent('message', event))
        }
      }
    ...
    function createPostMessageHandler (port1, port2) {
      port1[Symbol.for('oro.runtime.ipc.IPCMessagePort.handlePostMessage')] = (
        message,
        _,
        port
      ) => {
        const transferred =
          port2[Symbol.for('oro.runtime.ipc.IPCMessagePort.transferred')]
        const rx = port2[Symbol.for('oro.runtime.ipc.IPCMessagePort.rx')]
        const tx = port2[Symbol.for('oro.runtime.ipc.IPCMessagePort.tx')]

        if (transferred) {
          tx.postMessage({
            token: port[Symbol.for('oro.runtime.ipc.IPCMessagePort.token')],
            data: message
          })
        } else {
          rx.dispatchEvent(
    ...
    return {
      ...
      async handle (subscription, listener) {
        if (subscription?.id && --subscription.refs === 0) {
          await request('broadcast_channel.unsubscribe', {
            id: subscription.id
          })
        }
