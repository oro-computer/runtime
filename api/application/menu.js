...
export class Menu extends EventTarget {
  ...
  constructor (type) {
    super()
    this.#type = type
    this.#channel = new BroadcastChannel(`oro.runtime.application.menu.${type}`)
  ...
  constructor (sourceEventTarget = globalThis, options = null) {
    ...
    if (this.#tray) {
      this.#tray.addEventListener('menuitem', (event) => {
        this.dispatchEvent(
          new MenuItemEvent(event.type, event.data, this.#tray)
        )
      })
    ...
    if (this.#system) {
      this.#system.addEventListener('menuitem', (event) => {
        this.dispatchEvent(
          new MenuItemEvent(event.type, event.data, this.#system)
        )
      })
    ...
    if (this.#context) {
      this.#context.addEventListener('menuitem', (event) => {
        this.dispatchEvent(
          new MenuItemEvent(event.type, event.data, this.#context)
        )
      })
...
export async function setMenu (options, type) {
  ...
  if (type === 'system') {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (err) {
        ...
        return ipc.Result.from({
          err: new Error(`${err} on line ${lineNo}: "${lineText}"`)
        })
      }
    }
  }
  ...
  const command =
    type === 'tray' ? 'application.setTrayMenu' : 'application.setSystemMenu'
