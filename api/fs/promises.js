...
export async function lchmod (path, mode) {
  ...
  const result = await ipc.send('fs.lchmod', { path, mode })
...
export async function mkdtemp (prefix, options) {
  ...
  const result = await ipc.send('fs.mkdtemp', { prefix })
  if (result.err) throw result.err
...
export async function utimes (path, atime, mtime) {
  ...
  if (!Number.isFinite(a) || !Number.isFinite(m)) {
    throw new TypeError('time must be a number or Date')
  }
  const result = await ipc.send('fs.utimes', { path, atime: a, mtime: m })
  if (result.err) throw result.err
}
...
export async function futimes (fd, atime, mtime) {
  ...
  const result = await ipc.send('fs.futimes', {
    id: handle.id,
    atime: a,
    mtime: m
  })
...
export async function lutimes (path, atime, mtime) {
  ...
  const result = await ipc.send('fs.lutimes', { path, atime: a, mtime: m })
  if (result.err) throw result.err
}
...
export async function writev (fdOrHandle, buffers, position = null) {
  const handle =
    fdOrHandle instanceof FileHandle ? fdOrHandle : FileHandle.from(fdOrHandle)
  return await handle.writev(buffers, position)
}

/** Vector read: read into multiple buffers from a FileHandle or fd */
export async function readv (fdOrHandle, buffers, position = null) {
  const handle =
    fdOrHandle instanceof FileHandle ? fdOrHandle : FileHandle.from(fdOrHandle)
  return await handle.readv(buffers, position)
}

/** Truncate file */
export async function truncate (path, len = 0) {
  const handle = await FileHandle.open(path, 'r+')
  try {
  ...
  try {
    ...
    if (stats.isDirectory()) {
      ...
      for (const entry of entries) {
        ...
        await rm(joinPath(path, name), { recursive, force })
      }
      await (await import('./promises.js')).rmdir(path)
    } else {
