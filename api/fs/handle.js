...
export class FileHandle extends EventEmitter {
  ...
  async chown (uid, gid, options) {
    ...
    if (!Number.isInteger(uid) || !Number.isInteger(gid)) {
      throw new TypeError('uid and gid must be integers')
    }

    const result = await ipc.request(
      'fs.chown',
      { path: this.path, uid, gid },
      { ...options }
    )
    if (result.err) throw result.err
  }
  ...
  async datasync () {
    ...
    // Use fsync for now; native fdatasync not exposed
    const result = await ipc.request('fs.fdatasync', { id: this.id })
    if (result.err) throw result.err
  }
  ...
  async read (buffer, offset, length, position, options) {
    ...
    if (typeof offset !== 'number') {
      throw new TypeError(
        `Expecting offset to be a number. Got ${typeof offset}`
      )
    }

    if (typeof length !== 'number') {
      throw new TypeError(
        `Expecting length to be a number. Got ${typeof length}`
      )
    }
    ...
    if (offset + length > buffer.length) {
      throw new RangeError(
        'Offset + length cannot be larger than buffer length.'
      )
    }
  ...
  async readv (buffers, position = null) {
    ...
    for (const buf of buffers) {
      const { bytesRead } = await this.read({
        buffer: buf,
        offset: 0,
        length: buf.byteLength ?? buf.length,
        position: pos
      })
      total += bytesRead
      if (typeof pos === 'number') pos += bytesRead
  ...
  async writev (buffers, position = null) {
    ...
    for (const buf of buffers) {
      ...
      total += bytesWritten
      if (typeof pos === 'number') pos += bytesWritten
...
export class DirectoryHandle extends EventEmitter {
   ...
   */
  static get DEFAULT_BUFFER_SIZE () {
    return 32
  }
    ...
    if (this.#fileSystemHandle) {
      ...
      for (let i = 0; i < entries; ++i) {
        ...
        results.push({
          name,
          type:
            handle instanceof globalThis.FileSystemDirectoryHandle
              ? 'directory'
              : 'file'
        })
      }
