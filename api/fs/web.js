...
export const kFileSystemHandleFullName = Symbol.for(
  'oro.runtime.fs.web.FileSystemHandleFullName'
)
export const kFileDescriptor = Symbol.for('oro.runtime.fs.web.FileDescriptor')
export const kFileFullName = Symbol.for('oro.runtime.fs.web.FileFullName')
...
export const FileSystemFileHandle =
  ...
  class FileSystemFileHandle extends FileSystemHandle {
    async getFile () {}
    async createWritable (_options = null) {}
...
export const FileSystemDirectoryHandle =
  ...
  class FileSystemDirectoryHandle extends FileSystemHandle {
    ...
    async removeEntry (_name, _options = null) {}
    async getDirectoryHandle (_name, _options = null) {}
    async getFileHandle (_name, _options = null) {}
  }
...
function adapter (Adapter) {
  return Object.getOwnPropertyDescriptors(
    typeof Adapter === 'object' ? Adapter : Adapter.prototype
  )
}
...
export async function createFile (filename, options = null) {
  if (!globalThis.File) {
    console.warn("oro:fs/web: Missing platform 'File' implementation")
  }
...
export async function createFileSystemWritableFileStream (handle, options) {
  if (!globalThis.FileSystemWritableFileStream) {
    console.warn(
      "oro:fs/web: Missing platform 'FileSystemWritableFileStream' implementation"
    )
  }
      ...
      async write (options) {
        ...
        if (type === 'seek') {
          if (!Number.isFinite(position) || position < 0) {
            throw new TypeError(
              'Expecting options.position to be an unsigned integer'
            )
          }
        ...
        if (type === 'truncate') {
          if (!Number.isFinite(size) || size < 0) {
            throw new TypeError(
              'Expecting options.size to be an unsigned integer'
            )
          }
...
export async function createFileSystemFileHandle (file, options = null) {
  ...
  if (!globalThis.FileSystemFileHandle) {
    console.warn(
      "oro:fs/web: Missing platform 'FileSystemFileHandle' implementation"
    )
  }
  ...
  return create(
    ...
    class FileSystemFileHandle {
      ...
      async isSameEntry (entry) {
        ...
        if (
          this[kFileSystemHandleFullName] === entry[kFileSystemHandleFullName]
        ) {
          return true
        }
      ...
      async move (nameOrDestinationHandle, name = null) {
        ...
        if (typeof nameOrDestinationHandle === 'string') {
          ...
          nameOrDestinationHandle instanceof FileSystemDirectoryHandle
        ) {
          destination = nameOrDestinationHandle
        }

        const newName = path.resolve(
          destination?.[kFileSystemHandleFullName] ||
            this[kFileSystemHandleFullName],
          '..',
          name
        )

        await fs.rename(this[kFileSystemHandleFullName], newName)
...
export async function createFileSystemDirectoryHandle (dirname, options = null) {
  ...
  if (!globalThis.FileSystemDirectoryHandle) {
    console.warn(
      "oro:fs/web: Missing platform 'FileSystemDirectoryHandle' implementation"
    )
  }
  ...
  return create(
    ...
    class FileSystemDirectoryHandle {
      ...
      async move (nameOrDestinationHandle, name = null) {
        if (!writable) {
          throw new NotAllowedError(
            "FileSystemDirectoryHandle is in 'readonly' mode"
          )
        }
        ...
        const newName = path.resolve(
          destination?.[kFileSystemHandleFullName] || dirname,
          '..',
          name
        )
      ...
      async * entries () {
        ...
        for await (const entry of fd) {
          if (entry.isDirectory()) {
            ...
            yield [
              entry.name,
              await createFileSystemFileHandle(path.join(dirname, entry.name), {
                writable
              })
            ]
          }
      ...
      async resolve (possibleDescendant) {
        ...
        while (components.length) {
          ...
          const value =
            last && component ? path.join(last, component) : component
          if (value) {
            paths.push(value)
          }
        }
      ...
      async removeEntry (name, options = null) {
        ...
        if (!handle) {
          if (stats.isDirectory()) {
            handle = await createFileSystemDirectoryHandle(filename, {
              writable
            })
