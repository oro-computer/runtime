  ...
  static from (stat, fromBigInt = false) {
    if (fromBigInt) {
      return new this({
        ...
        atimeMs:
          BigInt(stat.st_atim?.tv_sec ?? 0n) * 1000n +
          BigInt(stat.st_atim?.tv_nsec ?? 0n) / 1000_000n,
        mtimeMs:
          BigInt(stat.st_mtim?.tv_sec ?? 0n) * 1000n +
          BigInt(stat.st_mtim?.tv_nsec ?? 0n) / 1000_000n,
        ctimeMs:
        ...
        birthtimeMs:
          BigInt(stat.st_birthtim?.tv_sec ?? 0n) * 1000n +
          BigInt(stat.st_birthtim?.tv_nsec ?? 0n) / 1000_000n,
        birthtimeNs:
          BigInt(stat.st_birthtim?.tv_sec ?? 0n) * 1000_000_000n +
          BigInt(stat.st_birthtim?.tv_nsec ?? 0n)
      })
    }
    ...
    return new this({
      ...
      atimeMs:
        Number(stat.st_atim?.tv_sec ?? 0) * 1000 +
        Number(stat.st_atim?.tv_nsec ?? 0) / 1000_000,
      mtimeMs:
        Number(stat.st_mtim?.tv_sec ?? 0) * 1000 +
        Number(stat.st_mtim?.tv_nsec ?? 0) / 1000_000,
      ctimeMs:
        Number(stat.st_ctim?.tv_sec ?? 0) * 1000 +
        Number(stat.st_ctim?.tv_nsec ?? 0) / 1000_000,
      birthtimeMs:
        Number(stat.st_birthtim?.tv_sec ?? 0) * 1000 +
        Number(stat.st_birthtim?.tv_nsec ?? 0) / 1000_000
    })
  }
