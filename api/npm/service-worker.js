...
import {
  PRIMARY_SCHEME,
  withPreferredRuntimeScheme
} from '../internal/runtime-schemes.js'
...
export async function onRequest (request, env, ctx) {
  ...
  ;(void env, ctx)
  const npmDebug =
    process.env.ORO_NPM_DEBUG || process.env.ORO_RUNTIME_NPM_DEBUG
  ...
  const origin = withPreferredRuntimeScheme(
    requestOrigin.replace('npm://', `${PRIMARY_SCHEME}://`),
    { url: true }
  )
  const referer = request.headers.get('referer')
  let specifier = url.pathname.replace('/oro/npm/', '')
  const importOrigins = new Set(
    url.searchParams
      .getAll('origin')
      .concat(url.searchParams.getAll('origin[]'))
  )

  importOrigins.add(
    withPreferredRuntimeScheme(
      url.origin.replace('npm://', `${PRIMARY_SCHEME}://`),
      { url: true }
    )
  )
  ...
  if (typeof specifier === 'string') {
    try {
      specifier = new URL(
        require.resolve(specifier, { type: 'module' })
      ).toString()
    } catch {}
  }
  ...
  if (referer && !referer.startsWith('blob:')) {
    ...
    if (URL.canParse(referer, origin)) {
      ...
      if (refererURL.href.endsWith('/')) {
        ...
        // Set has no push(); use add() and normalize to trailing slash origin
        importOrigins.add(new URL('./', refererURL).href)
      }
    }
  }
  ...
  for (const value of importOrigins) {
    if (value.startsWith('npm:')) {
      ...
      origins.push(
        new URL(
          value,
          `${PRIMARY_SCHEME}://${url.host}${url.pathname}`
        ).href.replace(/\/$/, '')
      )
    } else if (value.startsWith('/')) {
      ...
      origins.push(value)
      // @ts-ignore
    } else if (URL.canParse(`${PRIMARY_SCHEME}://${value}`)) {
      origins.push(`${PRIMARY_SCHEME}://${value}`)
    }
  }
  ...
  if (resolved.type === 'module') {
    ...
    const proxy = /^\s*(export\s*default)/gm.test(text)
      ? `
