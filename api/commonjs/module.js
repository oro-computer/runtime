...
import {
  globalPaths,
  createRequire as createRequireImplementation
} from './require.js'
...
export function CommonJSModuleScope (
  ...
  const crypto = require('oro:crypto')
  // eslint-disable-next-line no-unused-vars
  const { Buffer } = require('oro:buffer')

  // eslint-disable-next-line no-unused-expressions
  ;(void exports, require, module, __filename, __dirname)
  // eslint-disable-next-line no-unused-expressions
  ;(void _process, console, _global, crypto, Buffer)
 ...
 * @type {string}
 */
export const COMMONJS_WRAPPER =
  CommonJSModuleScope.toString().split(/'module code'/)
...
export class Module extends EventTarget {
  ...
  static get main () {
    ...
    const main = new Module(location.origin, {
      ...
      package: new Package(location.origin, {
        ...
        // eslint-disable-next-line
        description:
          packageInfo?.description || application.config.meta_description,
    ...
    if (globalThis.window && globalThis === globalThis.window) {
      try {
        const importmapScriptElement = globalThis.document.querySelector(
  ...
  constructor (url, options = null) {
    ...
    this.#package =
      options.package instanceof Package
        ? options.package
        : (this.#parent?.package ??
          new Package(options.name ?? this.#id, options.package))
