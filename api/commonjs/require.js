...
export function createRequire (options) {
  ...
  function resolve (input, options = null) {
    ...
    const origins = new Set(
      [].concat(options?.origins).concat(resolve.paths(input)).filter(Boolean)
    )
  ...
  function paths (input) {
    ...
    const origins = new Set(
      globalPaths.map((path) => new URL(path, location.origin).href)
    )
    ...
    const results = Array.from(origins)
      .map((origin) =>
        origin.endsWith(prefix) ? new URL(origin) : new URL(prefix, origin)
      )
      .map((url) => url.href)
