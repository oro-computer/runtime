...
export class Name {
  ...
  static parse (input, options = null) {
    ...
    if (pathname.startsWith('@')) {
      ...
      pathname = [organization || '', name, ...components.slice(2)]
        .filter(Boolean)
        .join('/')
  ...
  constructor (name, options = null) {
    ...
    if (typeof name === 'string' || name instanceof URL) {
      ...
      parsed = {
        ...
        // @ts-ignore
        url:
          name.url instanceof URL || URL.canParse(name.url)
            ? new URL(name.url)
            : null,
...
export class Package {
  ...
  constructor (name, options = null) {
    ...
    if (
      ...
      throw new TypeError(
        `Expecting 'name' to be a string or URL. Received: ${name}`
      )
    }
    ...
    this.#name = Name.from(name, {
      origin:
        options.origin ?? options.loader?.origin ?? this.#loader.origin ?? null,
    ...
    this.#type =
      options.type && /(commonjs|module)/.test(options.type)
        ? options.type
        : this.#type
    ...
    this.#version =
      options.version ?? this.#name.version ?? DEFAULT_PACKAGE_VERSION
    this.#description = options.description ?? ''
    this.#dependencies = new Dependencies(this)

    if (options.dependencies && typeof options.dependencies === 'object') {
      if (
        options.dependencies instanceof Dependencies ||
        typeof options.dependencies.entries === 'function'
      ) {
  ...
  resolve (pathname, options = null) {
    ...
    const extensions =
      extname !== '' && this.loader.extensions.has(extname)
        ? new Set([extname])
        : new Set(
          Array.from(options?.extensions ?? [])
            .concat('')
    ...
    for (const extension of extensions) {
      for (const key in this.#exports) {
        ...
        exports =
          exports?.browser ?? exports?.node ?? exports?.default ?? exports
