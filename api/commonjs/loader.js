...
import { withPreferredRuntimeScheme } from '../internal/runtime-schemes.js'
...
export class RequestStatus {
  ...
  set request (request) {
    ...
    if (!this.#status && request?.loader?.cache?.status?.has?.(request?.id)) {
      this.#status =
  ...
  load (options = null) {
    ...
    if (
      ...
      os.platform() !== 'android' &&
      /^(oro:|https:)/.test(this.id) &&
      this.#request.id.includes(
        `://${application.config.meta_bundle_identifier}`
      )
    ) {
      try {
        const id = this.#request.id.replace('https:', 'oro:')
        fs.accessSync(id)
      } catch {
    ...
    if (
      this.#request &&
      contentLocation &&
      URL.canParse(contentLocation, this.origin)
    ) {
...
export class Request {
  ...
  static from (json, options) {
    ...
    if (
      ...
      // @ts-ignore
      status = RequestStatus.from(json.status)
    }
  ...
  constructor (url, origin, options = null) {
    ...
    this.#status =
      options?.status instanceof RequestStatus
        ? options.status
  ...
  load (options = null) {
    ...
    if (
      ...
      try {
        let id = this.id.replace('https:', 'oro:')
        id = withPreferredRuntimeScheme(id, {
          url: true
        })
        fs.accessSync(id)
      } catch {
