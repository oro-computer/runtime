...
export class Storage extends EventTarget {
    ...
    this.#proxy = new Proxy(this.#context, {
      ...
      set: (_, property, value) => {
        ...
        if (this.database && this.database.opened) {
          this.forwardRequest(
            this.database.put(property, value, { durability: 'relaxed' })
          )
        }
        return true
      },
   ...
   */
  get name () {
    return `oro.runtime.commonjs.cache.storage(${this.#name})`
  }
  ...
  async sync (options = null) {
    ...
    for (const key of delta) {
      ...
      promises.push(
        this.forwardRequest(
          this.database.put(key, value, { durability: 'relaxed' })
        )
      )
    }
...
export class Snapshot {
  ...
  constructor () {
    for (const key in Cache.shared) {
      // @ts-ignore
      this.#data[key] = new Snapshot.Data(
        Object.fromEntries(Cache.shared[key].entries())
      )
    }
  }
...
export class Cache {
  ...
  static async restore (names) {
    ...
    for (const name of names) {
      promises.push(
        Storage.open({ name }).then((storage) => {
  ...
  constructor (name, options) {
    ...
    this.#channel = new BroadcastChannel(`oro.runtime.commonjs.cache.${name}`)
        ...
        for (const entry of data[CACHE_CHANNEL_MESSAGE_REPLICATE]) {
          if (!this.has(...entry)) {
            ...
            if (value?.__type__) {
              this.#data.set(
                key,
                this.#types.get(value.__type__).from(value, {
                  loader: this.#loader
                })
              )
            } else {
