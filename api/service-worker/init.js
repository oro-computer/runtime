...
export const channel = new BroadcastChannel('oro.runtime.serviceWorker')
  ...
  constructor (filename, options) {
    ...
    if (info.serializedWorkerArgs) {
      try {
        ...
        try {
          options.args = JSON.parse(
            decodeURIComponent(info.serializedWorkerArgs)
          )
        } catch {
          options.args = JSON.parse(
            decodeURIComponent(decodeURIComponent(info.serializedWorkerArgs))
          )
        }
      }
    ...
    super(filename, {
      ...
      ...options,
      [Symbol.for('oro.runtime.internal.worker.type')]: 'serviceWorker'
    })
...
export async function onRegister (event) {
  ...
  for (const worker of workers.values()) {
    if (worker.priority === 'high' && /high|default|low/i.test(info.priority)) {
      ...
      worker.priority === 'default' &&
      /default|low/i.test(info.priority)
    ) {
      await worker.ready
...
export function onNotificationShow (event, target) {
  for (const worker of workers.values()) {
    if (worker.info.id === event.data.registration.id) {
      ...
      notification.onclick = (event) => {
        const action =
          typeof event?.action === 'string'
            ? event.action
            : notification.action || ''
        const rawAction = notification.rawAction || ''
        worker.postMessage({
          notificationclick: {
      ...
      notification.onclose = (_event) => {
        notification.onclose = null
        notification.onclick = null
        const rawAction = notification.rawAction || 'dismiss'
        const action = (() => {
