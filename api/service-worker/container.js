...
const SERVICE_WORKER_WINDOW_PATH = `${location.origin}/oro/service-worker/index.html`
...
class ServiceWorkerContainerInternalState {
  currentWindow = null
  controller = null
  channel = new BroadcastChannel('oro.runtime.serviceWorker')
  ready = new Deferred()
  init = new Deferred()
...
async function preloadExistingRegistration (container) {
  ...
  if (registration) {
    if (registration.active) {
      if (application.config.webview_service_worker_mode === 'hybrid') {
        if (
          !internal.get(container).isRegistered &&
          !internal.get(container).isRegistering
        ) {
          container.register(registration.active.scriptURL, {
...
export class ServiceWorkerContainer extends EventTarget {
    ...
    internal.define(this, 'onmessageerror', {
      ...
      set (onmessageerror) {
        if (internal.get(this).onmessageerror) {
          this.removeEventListener(
            'messageerror',
            internal.get(this).onmessageerror
          )
        }
  ...
  async getRegistration (clientURL) {
    if (
      ...
      try {
        return await globalThis.top.navigator.serviceWorker.getRegistration(
          clientURL
        )
      } catch {}
    }
    ...
    if (globalThis.isWorkerScope) {
      if (globalThis.RUNTIME_WORKER_LOCATION.startsWith('blob:')) {
        currentScope = new URL(
          '.',
          new URL(globalThis.RUNTIME_WORKER_LOCATION).pathname
        ).pathname
      } else {
  ...
  async getRegistrations (options) {
    ...
    if (Array.isArray(result.data)) {
      for (const registration of result.data) {
        ...
        info.registration.state = info.registration.state.replace(
          'registered',
          'installing'
        )
  ...
  async register (scriptURL, options = null) {
    ...
    if (
      ...
      return await globalThis.top.navigator.serviceWorker.register(
        scriptURL,
        options
      )
    }
    ...
    const url = 'blob:'.startsWith(globalThis.location.origin)
      ? new URL(
        info.registration.scope,
        new URL(globalThis.location.origin).pathname
      )
    ...
    internal.get(this).channel.onmessage = async (event) => {
      if (
        ...
        for (const registration of registrations) {
          const info =
            registration[
              Symbol.for('oro.runtime.ServiceWorkerRegistration.info')
            ]
