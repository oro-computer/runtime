...
export const FETCH_EVENT_MAX_RESPONSE_REDIRECTS =
  // TODO(@jwerle): document this
  parseInt(
    application.config.webview_service_worker_fetch_event_max_response_redirects
...
export class ExtendableEvent extends Event {
   ...
   */
  get isActive () {
    return this.#pendingPromiseCount > 0 || this.eventPhase === Event.AT_TARGET
  }
    ...
    queueMicrotask(async () => {
      try {
        ...
        headerEntries = headerEntries.concat(
          Array.from(FetchEvent.defaultHeaders.entries())
        )
        const params = { statusCode, clientId, id }
