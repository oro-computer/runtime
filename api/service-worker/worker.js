...
export async function onMessage (event) {
  ...
  if (data?.register) {
    ...
    if (module.exports.default && typeof module.exports.default === 'object') {
      if (typeof module.exports.default.install === 'function') {
        state.install = module.exports.default.install.bind(
    ...
    if (module.exports.default && typeof module.exports.default === 'object') {
      if (typeof module.exports.default.activate === 'function') {
        state.activate = module.exports.default.activate.bind(
          module.exports.default
        )
      }
  ...
  if (data?.fetch?.request) {
    ...
    if (/post|put|patch|query/i.test(data.fetch.request.method)) {
      const result = await ipc.request(
        'serviceWorker.fetch.request.body',
        {
    ...
    const method = (data.fetch.request.method ?? 'GET').toUpperCase()
    const requestInit = {
    ...
    if (
      method !== 'GET' &&
      method !== 'HEAD' &&
      data.fetch.request.body !== undefined
    ) {
      requestInit.body = data.fetch.request.body
    }
    const fetchEvent = new FetchEvent('fetch', {
      clientId: data.fetch.client.id,
      fetchId: data.fetch.request.id,
      request: new Request(data.fetch.request.url, requestInit)
    })

    events.add(fetchEvent)
    if (protocolData) {
      fetchEvent.context.data = protocolData
    } else if (url.protocol !== 'oro:' && url.protocol !== 'npm:') {
      ...
      new NotificationEvent('notificationclick', {
        ...
        notification: new Notification(
          ...
          event.data.notificationclick.options,
          {
            ...
            action: event.data.notificationclick.rawAction
          }
        )
      })
    )
  ...
  if (event.data?.notificationclose) {
    event.stopImmediatePropagation()
    globalThis.dispatchEvent(
      new NotificationEvent('notificationclose', {
        ...
        notification: new Notification(
          ...
          event.data.notificationclose.options,
          {
            ...
            action: event.data.notificationclose.rawAction
          }
        )
      })
    )
