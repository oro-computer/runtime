...
export class Channel {
  #subscribers = Array.from({ length: MIN_CHANNEL_SUBSCRIBER_SIZE })
  #subscribed = 0
   ...
   */
  async publish (_name, _message = undefined) {
    return false
  }
   ...
   * @ignore
   */
  [Symbol.for('gc.finalizer')] (_options) {
    return {
...
export const registry = new (class ChannelRegistry {
   ...
   */
  channel (name) {
    return this.has(name) ? this.get(name) : this.set(name, new Channel(name))
  }
