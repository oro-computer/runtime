...
export class RemotePeer {
  ...
  async write (sharedKey, args) {
    ...
    for (const packet of packets) {
      ...
      this.localPeer._onDebug(
        `>> WRITE STREAM (from=${from}, to=${to}, via=${rinfo.address}:${rinfo.port})`
      )
...
export class Peer {
  ...
  ` /* snapshot_end=1691579150299 */
    ...
    (/** @type {object} */ o) => new RemotePeer({ ...o, indexed: true }, this)
  )
  ...
  constructor (persistedState = {}, dgram) {
    ...
    //
    // The purpose of this.config is to separate transitioned state from initial state.
    //
    this.config = {
      // TODO: consider freezing to guard against unintended mutation
  ...
  // Build a stable JSON representation for signing
  _stableStringify (value) {
    const sortKeys = (obj) => {
      if (!obj || typeof obj !== 'object') return obj
      if (Array.isArray(obj)) return obj.map(sortKeys)
      if (obj instanceof Uint8Array || obj?.buffer instanceof ArrayBuffer) {
        return Buffer.from(obj).toString('hex')
      }
      const o = {}
      for (const k of Object.keys(obj).sort()) o[k] = sortKeys(obj[k])
      return o
    }
    try {
  ...
  // Assemble canonical control-plane payload for signing
  _cpPayload (type, clusterId, subclusterId, message) {
    const hdr = Buffer.alloc(3)
    hdr.writeUInt8(type, 0)
    hdr.writeUInt16BE(VERSION, 1)
    const cid = Buffer.from(clusterId || [])
    const scid = Buffer.from(subclusterId || [])
    const m = Buffer.from(this._stableStringify(message || {}))
    return Buffer.concat([hdr, cid, scid, m])
  }

  // Optionally sign control-plane props using configured signingKeys
  _applyControlAuth (PacketCtor, props) {
    if (this.config?.controlPlaneAuth !== 'sig') return props
    const keys = this.config?.signingKeys
    if (!keys?.privateKey || !keys?.publicKey) return props
    const type = PacketCtor.type
    const clusterId =
      props.clusterId ?? this.config.clusterId ?? Buffer.from([0])
    const subclusterId = props.subclusterId ?? Buffer.from([0])
    const payload = this._cpPayload(
      type,
      clusterId,
      subclusterId,
      props.message
    )
    const sig = sodium.crypto_sign_detached(payload, keys.privateKey)
    props.sig = Buffer.from(sig)
    props.usr2 = Buffer.from(keys.publicKey)
    return props
  }

  // Verify control-plane signature if enabled
  _verifyControlAuth (packet) {
    if (this.config?.controlPlaneAuth !== 'sig') return true
    // only for control-plane types (exclude publish/stream)
    if (![1, 2, 3, 4, 8].includes(packet.type)) return true
    const pk = Buffer.from(packet.usr2 || [])
    const sig = Buffer.from(packet.sig || [])
    ...
    try {
      return sodium.crypto_sign_verify_detached(sig, payload, pk)
    } catch {
      return false
    }
  }
  ...
  async init (cb) {
    ...
    this.mainLoopTimer = this._setInterval(
  ...
  _scheduleSend () {
    ...
    this.sendTimeout = this._setTimeout(() => {
      this._dequeue()
    })
  }
  ...
  getPeers (packet, peers, ignorelist, filter = (o) => o) {
    ...
    if (friends.length) {
      ...
      list.unshift(
        ...candidates.filter(
          (c) =>
            c.address === friends[0].address && c.peerId === friends[0].peerId
        )
      )
    }
  ...
  async requestReflection () {
    ...
    if (peers.length < 2) {
      if (this.onConnecting) {
        this.onConnecting({ code: -1, status: 'Not enough pingable peers' })
      }
      ...
      for (const peer of this.peers
        ...
        await this.ping(peer, false, {
          message: { isConnection: true, requesterPeerId: this.peerId }
        })
      }
    ...
    if (this.reflectionStage === 0) {
      if (this.onConnecting) {
        this.onConnecting({ code: 1, status: 'Discover External Port' })
      }
    ...
    if (this.reflectionStage === 1) {
      ...
      if (!peer1 || !peer2) {
        ...
        if (this.onConnecting) {
          this.onConnecting({ code: 1.5, status: 'Insufficent Peers' })
        }
      ...
      this.reflectionTimeout = this._setTimeout((_ts) => {
        this.reflectionTimeout = null
  ...
  async ping (peer, withRetry, props, socket) {
    ...
    this._applyControlAuth(PacketPing, props)
  ...
  async join (sharedKey, args = { rateLimit: MAX_BANDWIDTH }) {
    ...
    this._applyControlAuth(PacketJoin, jprops)
    const packet = new PacketJoin(jprops)
  ...
  async _message2packets (T, message, args) {
    ...
    const siblings =
      ...
      [...this.cache.data.values()].filter(Boolean).filter((p) => {
        ...
        return (
          Buffer.from(p.previousId).compare(Buffer.from(packet.packetId)) === 0
        )
      })
  ...
  async publish (sharedKey, args) {
    ...
    if (!isBufferLike(args.message)) {
      throw new Error('.publish() will only accept a message of type buffer')
    }
  ...
  async query (query) {
    ...
    this._applyControlAuth(PacketQuery, qprops)
    const packet = new PacketQuery(qprops)
    const data = await Packet.encode(packet)
    ...
    this._onDebug(
      `-> QUERY (type=question, query=${query}, packet=${pid.slice(0, 8)})`
    )
  ...
  async _onQuery (packet, port, address) {
    ...
    this._onDebug(
      `<- QUERY (type=${type}, from=${address}:${port}, packet=${pid.slice(0, 8)})`
    )
    ...
    if (this.returnRoutes.has(queryId) && type === 'answer') {
      ...
      if (query.packetId && this.cache.has(query.packetId)) {
        ...
        if (q) {
          packets.push(
            ...(await this._message2packets(PacketQuery, q.message, q))
          )
        }
      }
  ...
  async _onPing (packet, port, address) {
    ...
    this._applyControlAuth(PacketPong, pprops)
    const packetPong = new PacketPong(pprops)
  ...
  async _onIntro (packet, port, address, _, opts = { attempts: 0 }) {
    ...
    if (opts.attempts === 0 && this.gate.has(peerId + peerAddress + peerPort)) {
      return
    }
    ...
    if (strategy === NAT.STRATEGY_TRAVERSAL_CONNECT) {
      ...
      const interval = this._setInterval(async () => {
        ...
        if (!pooledSocket) return // TODO recover from exhausted socket pool
        ...
        pooledSocket.on('message', async (msg, rinfo) => {
          ...
          this.socketPool[oldIndex] = this.dgram
            .createSocket('udp4', null, this)
            .unref()
    ...
    if (strategy === NAT.STRATEGY_TRAVERSAL_OPEN) {
      ...
      if (!this.bdpCache.length) {
        globalThis.bdpCache = this.bdpCache = Array.from({ length: 1024 }, () =>
  ...
  async _onJoin (packet, port, address, _data) {
    ...
    // If this peer is not a member of the target cluster, avoid cross-cluster introductions.
    // Still relay the JOIN to aid discovery, but do not introduce/connect.
    if (!this.clusters[cid]) {
      this._onDebug('== JOIN: not a member of cluster; relay only')
      this.gate.set(pid, 2)
      if (packet.hops < this.maxHops) {
        this.mcast(packet, [{ port, address }])
      }
      return
    }
    ...
    if (rendezvousDeadline && !this.indexed && this.clusters[cid]) {
      if (!packet.message.rendezvousRequesterPeerId) {
        ...
        this._applyControlAuth(PacketJoin, rprops)
        const data = await Packet.encode(new PacketJoin(rprops))
    ...
    if (
      ...
      if (peer.clusters && peer.clusters[cid]) {
        peers = [peer]
      } else {
        this._onDebug('== RENDEZVOUS SKIPPED: peer not in cluster')
        peers = []
      }
    ...
    for (const peer of peers) {
      ...
      const iprops1 = { ...opts, clusterId, subclusterId, message: message1 }
      const iprops2 = { ...opts, clusterId, subclusterId, message: message2 }
      this._applyControlAuth(PacketIntro, iprops1)
      this._applyControlAuth(PacketIntro, iprops2)
      const intro1 = await Packet.encode(new PacketIntro(iprops1))
      const intro2 = await Packet.encode(new PacketIntro(iprops2))
  ...
  _onProbeMessage (data, { port, address }) {
    ...
    if (!this._verifyControlAuth(packet)) {
      this._onDebug(
        `XX AUTH FAIL (probe, type=${packet.type}, from=${address}:${port})`
      )
      return
    }
  ...
  async _onMessage (data, { port, address }) {
    ...
    if (!this._verifyControlAuth(packet)) {
      this._onDebug(
        `XX AUTH FAIL (type=${packet.type}, from=${address}:${port})`
      )
      return
    }
    ...
    if (!this.config.limitExempt) {
      const quota = (subcluster && subcluster.rateLimit) || undefined
      if (rateLimit(this.rates, packet.type, port, address, quota)) {
        this._onDebug(
    ...
    switch (packet.type) {
      case PacketPing.type:
        return this._onPing(...args)
      case PacketPong.type:
        return this._onPong(...args)
    }
    ...
    switch (packet.type) {
      case PacketIntro.type:
        return this._onIntro(...args)
      case PacketJoin.type:
        return this._onJoin(...args)
      case PacketPublish.type:
        return this._onPublish(...args)
      case PacketStream.type:
        return this._onStream(...args)
      case PacketSync.type:
        return this._onSync(...args)
