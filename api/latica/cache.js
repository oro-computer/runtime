...
export class Cache {
  ...
  async compose (packet, source = this.data) {
    ...
    if (packet?.index > 0) {
      previous = source.get(packet.previousId?.toString('hex'))
    }
    ...
    let bufs = [...source.values()].filter((p) => {
      ...
      return (
        Buffer.from(p.previousId).compare(Buffer.from(previous.packetId)) === 0
      )
    })
    ...
    const packetId = await sha256(
      Buffer.concat([Buffer.from(packet.previousId || ''), message]),
      { bytes: true }
    )
  ...
  async sha1 (value, toHex) {
    const buf = await createDigest(
      'SHA-256',
      isBufferLike(value) ? value : Buffer.from(value)
    )
    if (!toHex) return buf
   ...
   *
   */
  async summarize (prefix = '', predicate = () => false) {
    ...
    const buckets = await Promise.all(
      children.map((child) => {
        return child.length
          ? this.sha1(child.sort().join(''), true)
  ...
  static encodeSummary (summary) {
    ...
    const prefixHex =
      summary.prefix.length % 2 ? '0' + summary.prefix : summary.prefix
    ...
    const bucketBin = Buffer.concat(
      summary.buckets.map((sum, offset) => {
  ...
  static decodeSummary (bin) {
    ...
    const prefix = bin
      .slice(o, (o += Math.ceil(plen / 2)))
      .toString('hex')
      .slice(-plen)

    // detect hash size per-frame for backward compatibility with SHA-1 summaries
    const remaining = bin.length - o
    const fitsSha1 = remaining >= 20 && (remaining - 20) % (1 + 20) === 0
    const fitsSha256 = remaining >= 32 && (remaining - 32) % (1 + 32) === 0
    const hashSize = fitsSha256 ? 32 : fitsSha1 ? 20 : Cache.HASH_SIZE_BYTES

    // hash is the summary hash (checksum of all other hashes)
    const hash = bin.slice(o, (o += hashSize)).toString('hex')

    // buckets are rows of { offset, sum } where the sum is not null
    const buckets = Array.from({ length: 16 }, () => null)
    while (o < bin.length) {
      const boff = bin.readUint8(o++)
      buckets[boff] = bin.slice(o, (o += hashSize)).toString('hex')
    }
  ...
  static isValidSummaryHashFormat (hash) {
    return (
      typeof hash === 'string' &&
      (/^[A-Fa-f0-9]{64}$/.test(hash) || /^[A-Fa-f0-9]{40}$/.test(hash))
    )
  }
}
