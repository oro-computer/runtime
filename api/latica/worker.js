...
globalThis.addEventListener('message', ({ data: source }) => {
  ...
  proxy = new PeerWorkerProxy(opts, source.port, async function ({
    data: args
  }) {
    const { prop, data, seq } = args
    ...
    switch (prop) {
      case 'create': {
        ...
        peer.onConnecting = (...args) =>
          this.callMainThread('onConnecting', args)
        peer.onConnection = (...args) =>
          this.callMainThread('onConnection', args)
        peer.onDisconnection = (...args) =>
          this.callMainThread('onDisconnection', args)
