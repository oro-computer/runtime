...
export const validateMessage = (o, constraints) => {
  ...
  if ({}.toString.call(constraints) !== '[object Object]') {
    throw new Error('expected constraints')
  }
  ...
  for (const [key, con] of Object.entries(constraints)) {
    ...
    if (type !== con.type) {
      ...
      throw new Error(
        `expected '${key}' to be of type '${con.type}', got '${type}'`
      )
    }
...
export const getTypeFromBytes = (buf) =>
  buf.byteLength > 4 ? buf.readUInt8(4) : 0
...
export class Packet {
  ...
  constructor (options = {}) {
    for (const [k, v] of Object.entries(PACKET_SPEC)) {
      this[k] = typeof options[k] === 'undefined' ? v.default : options[k]
  ...
  static async encode (p) {
    ...
    if (p.packetId.length === 1 && p.packetId[0] === 0) {
      const bufs = [p.previousId, p.message, p.nextId].map((v) =>
        Buffer.from(v)
      )
    ...
    for (const [k, spec] of Object.entries(PACKET_SPEC)) {
      ...
      if (spec.encoding === 'number') {
        ...
        if (bytesRequired > spec.bytes) {
          throw new Error(
            `key=${k}, value=${value} bytes=${bytesRequired}, max-bytes=${spec.bytes}, encoding=${spec.encoding}`
          )
        }
      ...
      if (value?.length && encoded.length > spec.bytes) {
        throw new Error(
          `${k} is invalid, ${value.length} is greater than ${spec.bytes} (encoding=${spec.encoding})`
        )
      }
  ...
  constructor (args) {
    ...
    validateMessage(args.message, {
      requesterPeerId: {
        required: true,
        type: 'string',
        assert: Peer.isValidPeerId
      },
      cacheSummaryHash: {
        type: 'string',
        assert: Cache.isValidSummaryHashFormat
      },
...
const isValidPort = (n) =>
  typeof n === 'number' && (n & 0xffff) === n && n !== 0
