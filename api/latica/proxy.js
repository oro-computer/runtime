/**
 * A utility to run the protocol in a thread separate from the UI.
 *
 * import { network } from
 *
 * Oro           Node
...
class PeerWorkerProxy {
  ...
  callWorkerThread (prop, data) {
    ...
    this.#channel.port1.postMessage({ prop, data, seq }, { transfer })
  ...
  callMainThread (prop, args) {
    for (const i in args) {
      ...
      if (
        arg?.constructor.name === 'RemotePeer' ||
        arg?.constructor.name === 'Peer'
      ) {
        args[i] = {
          // what details we want to expose outside of the protocol
          address: arg.address,
  ...
  resolveMainThread (seq, result) {
    if (result.err) {
      // err result of the worker try/catch
