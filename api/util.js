...
const kIgnoreInspect = (inspect.ignore = Symbol.for(
  'oro.runtime.util.inspect.ignore'
))
...
export const inspectSymbols = [
  Symbol.for('oro.runtime.util.inspect.custom'),
...
export function debug (section) {
  ...
  const sections = []
    .concat((env.ORO_DEBUG ?? '').split(','), (env.NODE_DEBUG ?? '').split(','))
...
export function isBoolean (value) {
  return (
    !isUndefined(value) &&
    !isNull(value) &&
    (value === true || value === false || value instanceof Boolean)
  )
}
...
export function toBuffer (object, encoding = undefined) {
  ...
  if (ArrayBuffer.isView(object)) {
    return Buffer.from(object.buffer, object.byteOffset, object.byteLength)
  }

  if (typeof object?.toBuffer === 'function') {
...
export function promisify (original) {
  ...
  let argumentNames = []
  // @ts-ignore
  if (Array.isArray(original[promisify.args])) {
    // @ts-ignore
    argumentNames = original[promisify.args]
  }
...
export function inspect (value, options) {
  const ctx = {
    ...
    customInspect:
      options?.customInspect === undefined ? true : options.customInspect,

    ...options,
    options: {
      stylize (label, _style) {
        return label
      },
      ...options
    }
  ...
  function formatValue (ctx, value, depth) {
    ...
    if (
      ...
      if (
        ...
        for (const inspectSymbol of inspectSymbols) {
          if (
            isFunction(value[inspectSymbol]) &&
            value[inspectSymbol] !== inspect
          ) {
            const formatted = value[inspectSymbol](depth, ctx.options, inspect)
    ...
    if (
      ...
      (isArrayLikeValue || value instanceof Set)
    ) {
      ...
      for (let i = 0; i < size; ++i) {
        ...
        if (value instanceof Set || hasOwnProperty(value, key)) {
          ...
          output.push(
            formatProperty(ctx, value, depth, enumerableKeys, key, true)
          )
        }
      }
      ...
      for (const key of keys) {
        if (
          ...
          output.push(
            formatProperty(ctx, value, depth, enumerableKeys, key, false)
          )
        }
      }
    } else {
    ...
    if (value instanceof Error) {
      ...
      if (
        value?.message &&
        !value?.stack?.startsWith?.(`${value?.name}: ${value?.message}`)
      ) {
      ...
      const formatWebkitErrorStackLine = (line) => {
        ...
        let output = []
        const root = new URL('../', import.meta.url || globalThis.location.href)
          .pathname

        let [context, lineno, colno] =
          maybeURL(location)?.pathname?.split(/:/) || location?.split(/:/) || []
        ...
        if (context) {
          ...
          if (/oro\//.test(context)) {
            context = context.replace('oro/', 'oro:').replace(/.js$/, '')
          }
        }
      ...
      out += (typeof value?.stack === 'string' ? value.stack : '')
        .split('\n')
        .map((line) =>
          line.includes(`${value.name}: ${value.message}`) ||
          /^\s*at\s/.test(line)
            ? line
...
export function format (format, ...args) {
  ...
  let str = format.replace(regex, (x) => {
    ...
    switch (x) {
      ...
      case '%o':
        return inspect(args[i++], { showHidden: true })
      case '%O':
...
export function parseHeaders (headers) {
  ...
  return headers
    ...
    .filter((e) => e.length >= 2)
    .map((e) => [
      e[0].trim().toLowerCase(),
      e.slice(1).join(':').trim().toLowerCase()
    ])
...
export const ESM_TEST_REGEX =
  /\b(import\s*[\w{},*\s]*\s*from\s*['"][^'"]+['"]|export\s+(?:\*\s*from\s*['"][^'"]+['"]|default\s*from\s*['"][^'"]+['"]|[\w{}*\s,]+))\s*(?:;|\b)/
...
export function deprecate (..._args) {
  // noop
}
