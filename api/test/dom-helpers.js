 ...
 * Example usage:
 * ```js
 * import { test } from 'oro:test/dom-helpers.js'
 * ```
 *
 */
// Based on vhs-tape.
...
export function toElement (selector) {
  if (globalThis.document) {
    ...
    if (
      ...
      throw new Error(
        'stringOrElement needs to be an instance of HTMLElement or a querySelector that resolves to a HTMLElement'
      )
    }
...
export function waitFor (args, lambda) {
  return new Promise((resolve, reject) => {
    const { selector, visible = true, timeout = defaultTimeout } = args
...
export function waitForText (args) {
  return waitFor(
    ...
    () => {
      const { element, text, regex, multipleTags } = args
...
export function event (args) {
  let { event, element = globalThis } = args
...
function isStyleVisible (element) {
  ...
  const { display, visibility, opacity } =
