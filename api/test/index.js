...
const { ORO_TEST_RUNNER_TIMEOUT = getDefaultTestRunnerTimeout() } = process.env

const TEST_FILTERS = createTestFilter(process.env)

/**
 * @typedef {Record<string, string | undefined>} TestProcessEnv
 */
...
export class Test {
  ...
  ifError (err, msg) {
    if (this.strict && !msg) throw new Error('tapzero msg required')
    this._assert(!err, err, 'no error', msg || String(err), 'ifError')
  }
  ...
  throws (fn, expected, message) {
    ...
    if (expected instanceof RegExp) {
      ...
      throw new Error(
        `t.throws() not implemented for expected: ${typeof expected}`
      )
    }
  ...
  async click (selector, msg) {
    msg =
      msg || `Clicked on ${typeof selector === 'string' ? selector : 'element'}`
    const el = toElement(selector)

    if (globalThis.HTMLElement && !(el instanceof globalThis.HTMLElement)) {
      throw new Error(
        'selector needs to be instance of HTMLElement or resolve to one'
      )
    }
  ...
  async eventClick (selector, msg) {
    ...
    msg =
      msg ||
      `Fired click event on ${typeof selector === 'string' ? selector : 'element'}`
    dispatchEventHelper({
  ...
  async dispatchEvent (event, target, msg) {
    ...
    msg =
      msg ||
      `Fired event ${typeof event === 'string' ? ` ${event}` : ''} on ${typeof target === 'string' ? target : 'element'}`
  ...
  async focus (selector, msg) {
    msg =
      msg || `Focused on ${typeof selector === 'string' ? selector : 'element'}`
    const el = toElement(selector)
    if (globalThis.HTMLElement && !(el instanceof globalThis.HTMLElement)) {
      throw new Error(
        'selector needs to be instance of HTMLElement or resolve to one'
  ...
  async blur (selector, msg) {
    msg =
      msg ||
      `Blurred from ${typeof selector === 'string' ? selector : 'element'}`
    const el = toElement(selector)
    if (globalThis.HTMLElement && !(el instanceof globalThis.HTMLElement)) {
      throw new Error(
        'selector needs to be instance of HTMLElement or resolve to one'
  ...
  async type (selector, str, msg) {
    msg =
      msg ||
      `Typed by value ${str}${typeof selector === 'string' ? ` to ${selector}` : ''}`
    const el = toElement(selector)
  ...
  async elementVisible (selector, msg) {
    msg =
      msg ||
      `Element ${typeof selector === 'string' ? ` to ${selector}` : ''} is visible`
    const el = toElement(selector)
  ...
  async elementInvisible (selector, msg) {
    msg =
      msg ||
      `Element ${typeof selector === 'string' ? ` to ${selector}` : ''} is not visible`
    const el = toElement(selector)
    const previousEl = el.previousElementSibling
    const visible = isElementVisible(el, previousEl)
    await this.requestAnimationFrame()
  ...
  waitFor (querySelectorOrFn, opts, msg) {
    ...
    if (!selector && !selectorFn) {
      throw new Error(
        'A query selector string or selector function is required'
      )
    }

    msg =
      msg ||
      `Waiting for element ${typeof selector === 'string' ? ` ${selector}` : ''}`
  ...
  waitForText (selector, opts, msg) {
    ...
    msg =
      msg ||
      `Waiting for text ${opts?.text ? ` ${opts.text}` : ''}${opts?.regex ? ` ${opts.regex}` : ''}`
  ...
  getComputedStyle (selector, msg) {
    msg =
      msg ||
      `Get computed style ${typeof selector === 'string' ? ` for ${selector}` : ''}`
    const el = toElement(selector)
...
export class TestRunner {
  ...
  add (name, fn, only) {
    ...
    if (this.filters) {
      if (!this._filtersAnnounced) {
        const description = this.filters.description
          ? `filters: ${this.filters.description}`
          : 'filters active'
        this.report(`# ${description}`)
        this._filtersAnnounced = true
      }
      if (!this.filters.shouldRun(name, { isOnly: !!only })) {
        this._filteredCount++
        return
      }
    }
  ...
  async run () {
    ...
    if (this._filteredCount) {
      this.report(`# filtered ${this._filteredCount}`)
    }
    if (fail) {
 ...
 * @ignore
 */
function createTestFilter (env) {
  const onlyPatterns = toRegexList(env.ORO_TEST_ONLY)
  const includePatterns = toRegexList(env.ORO_TEST_GREP || env.ORO_TEST_MATCH)
  const excludePatterns = toRegexList(env.ORO_TEST_SKIP)

  if (!onlyPatterns && !includePatterns && !excludePatterns) {
    return null
  }

  const parts = []
  if (onlyPatterns?.length) {
    parts.push(`only=${formatPatternList(onlyPatterns)}`)
  }
  if (includePatterns?.length) {
    parts.push(`match=${formatPatternList(includePatterns)}`)
  }
  if (excludePatterns?.length) {
    parts.push(`skip=${formatPatternList(excludePatterns)}`)
  }

  return {
    description: parts.join(' '),
    shouldRun (name) {
      if (onlyPatterns?.length && !onlyPatterns.some((rx) => rx.test(name))) {
        return false
      }
      if (
        includePatterns?.length &&
        !includePatterns.some((rx) => rx.test(name))
      ) {
        return false
      }
      if (
        excludePatterns?.length &&
        excludePatterns.some((rx) => rx.test(name))
      ) {
        return false
      }
      return true
...
function parsePattern (item) {
  if (item.startsWith('/') && item.lastIndexOf('/') > 0) {
    ...
    const body = item.slice(1, last)
    const flags = item.slice(last + 1) || 'i'
    try {
      return new RegExp(body, flags)
    } catch (err) {
      console.warn(
        `Warning: invalid RegExp pattern "${item}" ignored`,
        err?.message || err
      )
      return null
    }
...
function formatPatternList (patterns) {
  return patterns.map((rx) => `/${rx.source}/${rx.flags}`).join(',')
}
...
test.windows = test.win32 = function (name, fn) {
  if (os.platform() === 'win32') {
    return test(name, fn)
  }
}
...
test.iphone = test.ios = function (name, fn) {
  if (os.host() === 'iphoneos') {
    return test(name, fn)
  }
}

test.iphone.simulator = test.ios.simulator = function (name, fn) {
