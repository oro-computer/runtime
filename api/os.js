...
export function networkInterfaces () {
  ...
  for (const type in ipv4) {
    const info =
      typeof ipv4[type] === 'string' ? { address: ipv4[type] } : ipv4[type]
  ...
  for (const type in ipv6) {
    const info =
      typeof ipv6[type] === 'string' ? { address: ipv6[type] } : ipv6[type]
...
export function tmpdir () {
  ...
  if (isWindows()) {
    path =
      ...
      (globalThis.__args.env.SystemRoot ?? globalThis.__args.env.windir ?? '') +
        '\\temp'
