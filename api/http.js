...
export class OutgoingMessage extends Writable {
   ...
   */
  getRawHeaderNames () {
    return this.getHeaderNames().map((name) =>
      name.split('-').map(toProperCase).join('-')
    )
  }
...
export class IncomingMessage extends Readable {
  /**
   * Emitted whenever a piece of the message body is received.
   * @event IncomingMessage#data
   * @type {(chunk: Buffer) => void}
   */
  /**
   * Emitted when the entire message body has been received.
   * @event IncomingMessage#end
   * @type {() => void}
   */
  /**
   * Emitted when the underlying connection is terminated before the message ends.
   * @event IncomingMessage#aborted
   * @type {() => void}
   */
  /**
   * Emitted when the message times out.
   * @event IncomingMessage#timeout
   * @type {() => void}
   */
  #httpVersionMajor = 1
  #httpVersionMinor = 1
  ...
  constructor (options) {
    ...
    if (
      ...
      this.#statusMessage =
        options.statusMessage ?? STATUS_CODES[options.statusCode]
    }
   ...
   */
  get rawHeaders () {
    return Array.from(Object.entries(this.#headers)).reduce(
      (h, e) => h.concat(e),
      []
    )
  }
...
export class ClientRequest extends OutgoingMessage {
  /**
   * Emitted when a response is received for this request.
   * @event ClientRequest#response
   * @type {(res: IncomingMessage) => void}
   */
  /**
   * Emitted after the request has been sent and finished.
   * @event ClientRequest#finish
   * @type {() => void}
   */
  /**
   * Emitted when the request times out.
   * @event ClientRequest#timeout
   * @type {() => void}
   */
  /**
   * Emitted when an error occurs with the request.
   * @event ClientRequest#error
   * @type {(err: Error) => void}
   */
  #method = null
  #agent = null
  ...
  constructor (options) {
    ...
    if (options?.url) {
      ...
      if (url instanceof URL) {
        this.#url = url
        this.setHeader('host', url.hostname)
      } else {
        throw new TypeError('Invalid URL given')
      }
    }
   ...
   * @return {ClientRequest}
   */
  setSocketKeepAlive (_enable = false, _initialDelay = 0) {
    // not supported
    return this
  }
}
...
export class ServerResponse extends OutgoingMessage {
   ...
   * @event ServerResponse#finish
   * @type {() => void}
   */
  /**
   * Emitted when the connection is closed.
   * @event ServerResponse#close
   * @type {() => void}
   */
    ...
    function makeRequest (req) {
      ...
      request
        .then((response) => {
          ...
          return response.blob().then((blob) => blob.stream().getReader())
        })
        .then((reader) => {
          read()
          function read () {
...
export class Server extends EventEmitter {
   ...
   */
  /**
   * Emitted upon an incoming request.
   ...
   * @ignore
   */
  setTimeout (_timeout = 0, _callback = null) {
    // not supported
    return this
  }
...
/**
 * Makes a HTTP request, optionally a `oro://` for relative paths when
