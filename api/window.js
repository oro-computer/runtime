...
export function formatURL (url) {
  const href = location?.href ?? 'oro:///'
  const URLCtor = /** @type {any} */ (URL)
  return String(new URLCtor(url, href))
}
...
export class ApplicationWindow extends EventTarget {
  ...
  constructor ({ index, ...state }) {
    ...
    this.#index = index
    this.#state = state
    this.#channel = new BroadcastChannel(`oro.runtime.window.${this.#index}`)
  ...
  get followSystemTheme () {
    return this.#state.followSystemTheme ?? true
  }
   ...
   */
  get preferDarkTheme () {
    return this.#state.preferDarkTheme ?? false
  }
  ...
  get isDarkMode () {
    if (this.#state.isDarkMode === undefined) {
      return this.followSystemTheme ? null : Boolean(this.preferDarkTheme)
    }
    ...
    return Boolean(this.#state.isDarkMode)
  }
  ...
  get appearance () {
    return {
      ...
      preferDarkTheme: this.preferDarkTheme,
   ...
   */
  async show () {
    const response = await ipc.request('window.show', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
   ...
   */
  async hide () {
    const response = await ipc.request('window.hide', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
    return this.#updateState(response)
   ...
   */
  async focus () {
    const response = await ipc.request('window.focus', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
    return this.#updateState(response)
   ...
   */
  async blur () {
    const response = await ipc.request('window.blur', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
    return this.#updateState(response)
   ...
   */
  async maximize () {
    const response = await ipc.request('window.maximize', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
    return this.#updateState(response)
  ...
  async setTitle (title) {
    const response = await ipc.request('window.setTitle', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index,
      value: title
    })
  ...
  async setSize (opts) {
    ...
    if (
      ...
      (typeof opts.height === 'string' &&
        !isValidPercentageValue(opts.height)) ||
      (typeof opts.height === 'number' &&
  ...
  async setPosition (opts) {
    ...
    if (
      (opts.x != null &&
        ...
        typeof opts.x !== 'string') ||
      (typeof opts.x === 'string' && !isValidPercentageValue(opts.x)) ||
      (typeof opts.x === 'number' && !Number.isInteger(opts.x))
    ) {
      throw new Error(
        `Window x must be an integer number or a string with a valid percentage value from 0 to 100 ending with %. Got ${opts.x} instead.`
      )
    }
    ...
    if (
      (opts.y != null &&
        ...
        typeof opts.y !== 'string') ||
      (typeof opts.y === 'string' && !isValidPercentageValue(opts.y)) ||
      (typeof opts.y === 'number' && !Number.isInteger(opts.y))
    ) {
      throw new Error(
        `Window y must be an integer number or a string with a valid percentage value from 0 to 100 ending with %. Got ${opts.y} instead.`
      )
    }
   ...
   */
  async navigate (path) {
    const response = await ipc.request('window.navigate', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index,
      url: formatURL(path)
    })
   ...
   */
  async showInspector () {
    const { data, err } = await ipc.request('window.showInspector', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
    if (err) {
  ...
  async setBackgroundColor (opts) {
    const response = await ipc.request('window.setBackgroundColor', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index,
      ...opts
    })
  ...
  async getBackgroundColor () {
    const result = await ipc.request('window.getBackgroundColor', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
    if (result.err) {
    ...
    /** @type {string} */
    const color = /** @type {any} */ (result.data)
    return color
  }
  ...
  async setAlwaysOnTop (enabled) {
    const response = await ipc.request('window.setAlwaysOnTop', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index,
      value: enabled === true
    })
   ...
   */
  async isAlwaysOnTop () {
    const result = await ipc.request('window.getAlwaysOnTop', {
      index: this.#senderWindowIndex,
      targetWindowIndex: this.#index
    })
    if (result.err) {
  ...
  async share (options = {}) {
    ...
    if (options.text != null) {
      payload.text = String(options.text)
    }

    if (options.url != null) {
      payload.url = String(options.url)
    }

    const result = await ipc.request('window.share', payload)
  ...
  addListener (event, cb) {
    if (this.#index !== this.#senderWindowIndex) {
      throw new Error(
        'window.addListener can only be used from the current window'
      )
    }
  ...
  removeListener (event, cb) {
    if (this.#index !== this.#senderWindowIndex) {
      throw new Error(
        'window.removeListener can only be used from the current window'
      )
    }
  ...
  removeAllListeners (event) {
    if (this.#index !== this.#senderWindowIndex) {
      throw new Error(
        'window.removeAllListeners can only be used from the current window'
      )
    }
