...
export class CoreAsyncResource {
  ...
  constructor (type, options = null) {
    if (!type || typeof type !== 'string') {
      throw new TypeError(`Expecting 'type' to be a string. Received: ${type}`)
    }
    ...
    if (
      Number.isFinite(options?.triggerAsyncId) &&
      options.triggerAsyncId > 0
    ) {
