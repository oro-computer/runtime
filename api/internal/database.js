    ...
    const promise = new Promise((resolve, reject) => {
      ...
      function onerror (event) {
        ...
        const error = new Error(message, {
          cause: request.error ?? event.target
        })
      ...
      function onupgradeneeded (event) {
        const conflict = new DatabaseRequestQueueRequestConflict(
          resolve,
          reject,
          cleanup
        )
...
export class Database extends EventTarget {
  ...
  async open () {
    ...
    this.#storage = await queue.push(
      request,
      async (conflictEvent, conflict) => {
        if (conflictEvent.type === 'blocked') {
          // TODO(@jwerle): handle a 'blocked' event
          conflict.reject(
            new Error('Database: storage cannot be opened (blocked)')
          )
          ...
          conflict.reject(
            new Error('Database: storage cannot be opened (unknown reason)')
          )
        }
      }
    )
  ...
  async get (key, options = null) {
    ...
    const count =
      Number.isFinite(options?.count) && options.count > 0 ? options.count : 1
