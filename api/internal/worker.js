...
export const WorkerGlobalScopePrototype =
  globalThis.WorkerGlobalScope?.prototype ?? {}
 ...
 */
export const source =
  globalThis.RUNTIME_WORKER_LOCATION ?? url.searchParams.get('source') ?? null
 ...
 * @type {string}
 */
export const RUNTIME_WORKER_ID =
  globalThis.RUNTIME_WORKER_ID ?? rand64().toString()
...
if (isWorkerScope) {
  ...
  globalThis.addEventListener(
    'runtime-queued-response',
    function onRuntimeQueuedResponse (event) {
      if (isClosed) {
        globalThis.removeEventListener(
          'runtime-queued-response',
          onRuntimeQueuedResponse
        )
        return false
      }
...
export async function onWorkerMessage (event) {
  ...
  if (typeof data?.__runtime_worker_ipc_result === 'object') {
    ...
    if (resultData.message && resultData.result) {
      ...
      globalThis.dispatchEvent(
        new CustomEvent(`resolve-${index}-${seq}`, {
    ...
    const sharedWorker = data?.__runtime_shared_worker
    if (!sharedWorker || typeof sharedWorker !== 'object') {
      return true
    }
    for (const port of sharedWorker.ports || []) {
...
export function addEventListener (eventName, callback, ...args) {
  if (eventName === 'message') {
    return workerGlobalScopeEventTarget.addEventListener(
      eventName,
      callback,
      ...args
    )
...
export function removeEventListener (eventName, callback, ...args) {
  if (eventName === 'message') {
    return workerGlobalScopeEventTarget.removeEventListener(
      eventName,
      callback,
      ...args
    )
