...
import TCPServerSocket from './tcp-server-socket.js'
...
function ensureRuntimeDetectionGlobals () {
  const oroDescriptor = Object.getOwnPropertyDescriptor(
    globalThis,
    'isOroRuntime'
  )
  if (!oroDescriptor || oroDescriptor.configurable) {
...
export function init () {
  ...
  function install (implementations, target = globalThis, prefix = null) {
    for (let name in implementations) {
      ...
      if (
        typeof target[actualName] === 'object' &&
        target[actualName] !== null
      ) {
        for (const key in implementation) {
          ...
          if (nativeImplementation !== null) {
            const nativeName = ['_', 'native', ...name.split('.'), key].join(
              '_'
            )
        ...
        if (nativeImplementation !== implementation) {
          ...
          if (
            typeof nativeImplementation === 'function' &&
            nativeImplementation.prototype &&
            typeof implementation === 'function' &&
            implementation.prototype
          ) {
            const nativeDescriptors = Object.getOwnPropertyDescriptors(
              nativeImplementation.prototype
            )
  ...
  if (globalThis.navigator) {
    ...
    // @ts-ignore
    install({ credentials }, globalThis.navigator, 'credentials')
