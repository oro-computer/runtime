...
export async function getCurrentPosition (
  ...
  if (isAndroid) {
    ...
    const result = await ipc.request('permissions.request', {
      name: 'geolocation'
    })
 ...
 * @return {number}
 */
export function watchPosition (onSuccess, onError, options = { timeout: null }) {
  if (isAndroid) {
  ...
  ipc
    .request('geolocation.watchPosition', { id: identifier })
    .then((result) => {
      if (result.err) {
        if (typeof onError === 'function') {
          onError(result.err)
