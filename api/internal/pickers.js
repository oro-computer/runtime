...
export class Database {
  get (key) {
    try {
      return JSON.parse(
        globalThis.localStorage.getItem(`internal:pickers:${key}`)
      )
    } catch {
...
function requireUserActivation (which) {
  ...
  const requiresUserActivation =
    String(
      application.config.webview_filesystem_picker_require_user_activation
    ) !== 'false'
...
export async function showSaveFilePicker (options = null) {
  ...
  const { startIn, id, types, excludeAcceptAllOption, suggestedName } =
    options || {}
  const currentWindow = await application.getCurrentWindow()
