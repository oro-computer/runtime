      ...
      error:
        this.error === null
          ...
          : {
              ...
              stack: String(
                this.error[CallSite.StackSourceSymbol] ?? this.error.stack ?? ''
              )
            }
    }
  }
...
export class CallSite {
   ...
   * @type {symbol}
   */
  static PromiseElementIndexSymbol = Symbol.for(
    'oro.runtime.CallSite.PromiseElementIndex'
  )
   ...
   * @type {symbol}
   */
  static PromiseAllSymbol = Symbol.for('oro.runtime.CallSite.PromiseAll')
   ...
   * @type {symbol}
   */
  static PromiseAnySymbol = Symbol.for('oro.runtime.CallSite.PromiseAny')
   ...
   * @type {symbol}
   */
  static StackSourceSymbol = Symbol.for('oro.runtime.CallSite.StackSource')
  ...
  getFileName () {
    if (this.#frame.location.sourceURL) {
      ...
      let filename = new URL(this.#frame.location.sourceURL).pathname.replace(
        root,
        ''
      )

      if (/^\/?(oro|socket)\//.test(filename)) {
        filename = filename
          .replace(/^\/?oro\//, 'oro:')
          .replace(/^\/?socket\//, 'oro:')
          .replace(/\.js$/, '')
  ...
  getScriptNameOrSourceURL () {
    const url = new URL(
      this.#frame.location.sourceURL ?? globalThis.location.href
    )
  ...
  getScriptHash () {
    return Buffer.from(
      String(murmur3(this.getScriptNameOrSourceURL()))
    ).toString('hex')
  }
  ...
  getEnclosingLineNumber () {
    if (this.#previous) {
      ...
      if (
        previousSourceURL &&
        previousSourceURL === this.getScriptNameOrSourceURL()
      ) {
  ...
  isEval () {
    ...
    while (current) {
      if (
        current.frame.symbol === 'eval' ||
        current.frame.symbol === 'eval code'
      ) {
        return true
      }
  ...
  toString () {
    ...
    if (location.sourceURL) {
      ...
      if (location.lineNumber && location.columnNumber) {
        output.push(
          `(${filename}:${location.lineNumber}:${location.columnNumber})`
        )
