...
const kContextTag = Symbol('oro.vm.Context')
const kWorkerContextReady = Symbol('oro.vm.ContextWorker.ready')
 ...
 */
export const channel = new BroadcastChannel('oro.runtime.vm')
                          ...
                          .then((result) => {
                            if (result?.value) {
                              ...
                              if (
                                ...
                                property in result.value
                              ) {
                                if (
                                  typeof result.value[property] === 'function'
                                ) {
...
export class Reference {
  ...
  static isReference (value) {
    ...
    if (
      value?.__vmScriptReference__ === true &&
  ...
  constructor (id, value, context = null, options) {
    ...
    this.#context =
      context !== null && context !== undefined ? new WeakRef(context) : null
...
export class Script extends EventTarget {
  ...
  async runInContext (context, options = null) {
    ...
    const mode =
      options?.type ?? options?.mode ?? detectFunctionSourceType(source)
    const id = this.#id
  ...
  async runInNewContext (context, options = null) {
    ...
    const initialContext = context ?? this.context ?? {}
    const contextReference = initialContext
...
export async function getContextWindow () {
  ...
  const vmDebug = process.env.ORO_VM_DEBUG || process.env.ORO_RUNTIME_VM_DEBUG
...
export function createGlobalObject (context, options = null) {
  ...
  const traps = {
    ...
    ownKeys (_) {
      ...
      keys.push(
        ...Reflect.ownKeys(globalObject).filter((key) => {
...
export function compileFunction (source, options = null) {
  ...
  const globalObject =
    globalObjects.get(options?.context) ?? createGlobalObject(options?.context)

  const wrappedSource =
    options?.wrap === false ? source : wrapFunctionSource(source, options)
  ...
  if (options?.async === true) {
    // @ts-ignore
    compiled = new AsyncFunction(...args)
  } else {
    // @ts-ignore
    compiled = new Function(...args)
  }
...
export async function runInContext (source, context, options) {
  source = convertSourceToString(source)
  context = context?.context ?? options?.context ?? context ?? sharedContext
