...
/**
 * @typedef {import('./internal/events.js').ApplicationURLEvent} ApplicationURLEvent
 */
    ...
    try {
      ...
      addEventListener(globalObject, type, (event) => {
        ...
        if (type === 'applicationurl') {
          ...
          const {
            message,
            filename = import.meta.url || globalThis.location.href
          } = error || {}
          dispatchEvent(
            target,
            new ErrorEvent(type, {
  ...
  async wait (nameOrFunction, options = null) {
    ...
    if (typeof nameOrFunction === 'string') {
      ...
      if (descriptor?.value === nameOrFunction) {
        return await new Promise((resolve) => {
          const removeEventListener = /** @type {function} */ (
            nameOrFunction
          ).call(this, resolve)
