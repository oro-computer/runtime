...
export const SHARE_ENV = Symbol.for('oro.runtime.worker_threads.SHARE_ENV')
...
export const state = {
  ...
  parentPort: isMainThread ? null : Object.create(MessagePort.prototype),

  mainPort: isMainThread ? Object.create(MessagePort.prototype) : null,
...
if (!isMainThread) {
  ...
  globalThis.addEventListener('error', (event) => {
    propagateWorkerError(
      event.error ??
        new Error(
          event.reason?.message ?? event.reason ?? 'An unknown error occurred'
        )
    )
  })
