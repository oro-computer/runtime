...
import { debug as createDebug } from './util.js'

const debug = createDebug('conduit')
 ...
 * restart Conduit. Restrict lifecycle handling to mobile platforms.
 * @type {boolean}
 */
...
if (shouldHandleLifecycle) {
  hooks.onApplicationResume(async () => {
    ...
    debug('HOOKS_CONDUIT_STARTING', result?.data)
  ...
  hooks.onApplicationPause(async () => {
    ...
    debug('HOOKS_CONDUIT_SHOULD_STOP', result)
...
export class Conduit extends EventTarget {
  ...
  constructor (options) {
    ...
    debug('INTERNAL_CREATE CONDUIT CLIENT', this.id)
  ...
  async reconnect (options = {}) {
    ...
    try {
      ...
      debug(`Waiting ${Math.round(waitTime)}ms before next reconnect...`)
  ...
  encodeMessage (options, payload) {
    const headerBuffers = Object.entries(options).map(([key, value]) =>
      this.encodeOption(key, value)
    )

    const totalOptionLength = headerBuffers.reduce(
  ...
  decodeMessage (data) {
    ...
    for (let i = 0; i < numOpts; i++) {
      ...
      const key = textDecoder.decode(
        new Uint8Array(data.buffer, offset, keyLength)
      )
      offset += keyLength
      ...
      const value = textDecoder.decode(valueBuffer)
      options[key] = value
   ...
   */
  receive (callback) {
    if (typeof this._receiveCleanup === 'function') {
      this._receiveCleanup()
      this._receiveCleanup = null
    }
