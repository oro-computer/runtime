...
const NativeNotification =
  globalThis.Notification || class NativeNotification extends EventTarget {}
      ...
      notification.dispatchEvent(
        new ErrorEvent('error', {
          error:
            event.error ||
            event.message ||
            new Error('An unknown error occurred', { cause: event })
        })
      )
    }
 ...
 * @ignore
 */
export const NotificationDirection = Enumeration.from(['auto', 'ltr', 'rtl'])
...
export async function showNotification (
  ...
  await new Promise((resolve, reject) => {
    const cleanup = () => {
      notification.removeEventListener('show', handleShow)
    ...
    if (typeof onshow === 'function') {
      notification.addEventListener('show', onshow, { once: true })
    }

    notification.addEventListener('show', handleShow, { once: true })
    notification.addEventListener('error', handleError, { once: true })
  })
}
...
/**
 * Normalize platform-specific action keywords to the web API contract.
 * @ignore
 * @param {string} value
 * @return {string}
 */
...
/**
 * Create an event for notification observers and decorate it with the action.
 * @ignore
 * @param {string} type
 * @param {string=} action
 * @return {Event}
 */
function createNotificationEvent (type, action = '') {
...
/**
 * Serialize notification data for the native layer.
 * @ignore
 * @param {?any} value
 * @return {string}
 */
function serializeNotificationData (value) {
...
export class Notification extends EventTarget {
  ...
  static get permission () {
    if (typeof globalThis.__native_Notification?.permission === 'string') {
      if (state.permission === 'default') {
        state.permission =
          globalThis.__native_Notification.permission || state.permission
      }
    }
    return state.permission === 'prompt' ? 'default' : state.permission
  }
  ...
  constructor (title, options = {}, existingState = null) {
    ...
    try {
      ...
      throw new TypeError(`Failed to construct 'Notification': ${err.message}`)
    }
    ...
    if (existingState?.action) {
      this.#rawAction = String(existingState.action)
      this.#lastAction = normalizeAction(this.#rawAction)
    }

    const channel = new BroadcastChannel('oro.runtime.notification')
    ...
    if (existingState) {
      channel.addEventListener('message', async (event) => {
        if (event.data?.id === this.#id) {
          if (!this.#closed) {
            ...
            const detailAction = event.data?.detail?.action ?? ''

            if (event.data.action === 'click') {
              this.#lastAction = detailAction
              this.#rawAction = detailAction === '' ? 'default' : detailAction
            } else if (event.data.action === 'close' && detailAction) {
              this.#lastAction = detailAction
              this.#rawAction =
                detailAction === 'dismiss' ? 'dismiss' : detailAction
            }

            this.dispatchEvent(
              createNotificationEvent(event.data.action, detailAction)
            )
          }
        }
      })
      ...
      if (isLinux) {
        ...
        this.#proxy = proxy
        this[Symbol.for('oro.runtime.Notification.request')] = request
      } else {
        const request = ipc.request('notification.show', {
          ...
          tag: this.tag || '',
          silent: this.silent,
          vibrate: Array.isArray(this.vibrate) ? this.vibrate.join(',') : '',
          renotify: this.renotify,
          requireInteraction: this.requireInteraction,
          data: serializeNotificationData(this.data),
          actions: JSON.stringify(
            this.actions.map((action) => action.toJSON())
          ),
          timestamp: this.timestamp.toString(10),
          category: this.actions.length ? this.tag || this.#id : ''
        })
      ...
      const removeNotificationPresentedListener = hooks.onNotificationPresented(
        (event) => {
      ...
      const removeNotificationResponseListener = hooks.onNotificationResponse(
        (event) => {
          if (event.detail.id === this.id) {
            this.#closed = true
            const rawAction = event.detail.action
            const eventName = rawAction === 'dismiss' ? 'close' : 'click'
            ...
            state.pending.delete(this.id)
            this.dispatchEvent(
              createNotificationEvent(eventName, normalizedAction)
            )

            if (eventName === 'click') {
              queueMicrotask(() => {
                this.dispatchEvent(
                  createNotificationEvent('close', normalizedAction)
                )
                channel.postMessage({
                  id: this.id,
                  action: 'close',
      ...
      this[Symbol.for('oro.runtime.Notification.request')].then((result) => {
        if (result?.err) {
          // @ts-ignore
          state.pending.delete(this.id)
          removeNotificationPresentedListener()
      ...
      channel.addEventListener('message', async (event) => {
        if (event.data?.id === this.#id) {
          if (!this.#closed) {
            ...
            const rawAction =
              event.data?.detail?.rawAction ??
              (event.data.action === 'close' ? 'dismiss' : 'default')
            const detailAction =
              event.data?.detail?.action ??
  ...
  async close () {
    ...
    this.#closed = true
    this.#rawAction = 'dismiss'
    this.#lastAction = 'dismiss'
