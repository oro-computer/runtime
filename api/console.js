...
function isPatched (console) {
  return console?.[Symbol.for('oro.runtime.console.patched')] === true
}
  ...
  if (!columns) {
    ...
    if (Array.isArray(data)) {
      ...
      if (!isObject(first) && first !== undefined) {
        ...
        const keys = new Set(
          data.map(Object.keys).reduce((a, b) => a.concat(b), [])
        )
      ...
      for (const part of parts) {
        ...
        while (pending.length) {
          ...
          try {
            ...
            this.console?.warn?.(
              `Failed to write to ${destination}: ${err.message}`
            )
            return
          }
        }
...
export function patchGlobalConsole (globalConsole, options = {}) {
  ...
  if (!isPatched(globalConsole)) {
    ...
    globalConsole[Symbol.for('oro.runtime.console.patched')] = true
