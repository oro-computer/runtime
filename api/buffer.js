...
function byteLength (string, encoding) {
  ...
  for (;;) {
    switch (encoding) {
      ...
      case 'base64url':
        return base64ToBytes(base64UrlDecode(string)).length
      default:
        if (loweredCase) {
...
function base64urlWrite (buf, string, offset, length) {
  return base64Write(buf, base64UrlDecode(string), offset, length)
}
...
function base64urlSlice (buf, start, end) {
  return base64UrlEncode(base64Slice(buf, start, end))
}
...
function checkIEEE754 (buf, value, offset, ext, _max, _min) {
  if (offset + ext > buf.length) throw new RangeError('Index out of range')
...
function base64UrlDecode (str) {
  return String(str).replace(/-/g, '+').replace(/_/g, '/').replace(/\./g, '=')
}
